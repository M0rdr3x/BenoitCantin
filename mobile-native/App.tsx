...
export default function App() {
  ...
  const unlockWithBiometrics = async () => {
    try {
      ...
      const result = await LocalAuthentication.authenticateAsync({promptMessage:'Déverrouiller SINJIRA',cancelLabel:'Annuler',disableDeviceFallback:false});
      if (result.success) {setNativeMessage('');setIsUnlocked(true);return true;}
      setIsUnlocked(false);setNativeMessage('SINJIRA reste verrouillé. Réessayez lorsque vous êtes prêt.');return false;
    } catch {
      setIsUnlocked(false);setNativeMessage('La vérification locale est temporairement indisponible.');return false;
    }
  };
  ...
  const enableSecurityPush = async (quiet = false) => {
    ...
    setPushEnabled(true);setPushToken(token);syncPushToWeb(true,token);
  ...
  const disableSecurityPush = async () => {
    ...
    setPushEnabled(false);setPushToken('');syncPushToWeb(false,'');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!key) {key = makeDeviceKey();await SecureStore.setItemAsync(DEVICE_KEY_STORAGE,key);}
      ...
      if (cancelled) return;
      setNativeDeviceKey(key);setBiometricEnabled(biometric);setPushEnabled(push);setPushToken(token);
      if (biometric) await unlockWithBiometrics(); else setIsUnlocked(true);
      if (!cancelled) setSecurityReady(true);
      if (push && !token) void enableSecurityPush(true);
    })().catch(() => {
      if (!cancelled) {setNativeMessage('Le stockage sécurisé local est indisponible.');setIsUnlocked(true);setSecurityReady(true);}
    });
    return () => {cancelled=true};
  }, []);

  useEffect(() => {
    const subscription=AppState.addEventListener('change',(nextState)=>{const previous=appStateRef.current;appStateRef.current=nextState;if(!biometricEnabled)return;if(nextState==='background'||nextState==='inactive')setIsUnlocked(false);if((previous==='background'||previous==='inactive')&&nextState==='active')void unlockWithBiometrics();});
    return()=>subscription.remove();
  },[biometricEnabled]);

  useEffect(() => {
    Linking.getInitialURL().then((url)=>{const normalized=normalizeSinjiraUrl(url);if(normalized){setCurrentUrl(normalized);setActiveTab(tabForUrl(normalized)??'feed')}});
    const subscription=Linking.addEventListener('url',({url})=>{const normalized=normalizeSinjiraUrl(url);if(normalized){setCurrentUrl(normalized);setActiveTab(tabForUrl(normalized)??'feed')}});
    return()=>subscription.remove();
  },[]);
  ...
  useEffect(() => {
    const subscription=Notifications.addNotificationResponseReceivedListener((response)=>{
      ...
      if(typeof path==='string'&&path.startsWith('/')&&!path.startsWith('//'))setCurrentUrl(`${ORIGIN}${path}`);
    });
    return()=>subscription.remove();
  },[]);
  ...
  useEffect(() => {
    ...
    const subscription=BackHandler.addEventListener('hardwareBackPress',()=>{if(!isUnlocked&&biometricEnabled)return true;if(canGoBack){webViewRef.current?.goBack();return true}return false});
    return()=>subscription.remove();
  },[canGoBack,isUnlocked,biometricEnabled]);

  const toggleBiometric=async()=>{
    if(biometricEnabled){await SecureStore.deleteItemAsync(BIOMETRIC_LOCK_STORAGE);setBiometricEnabled(false);setIsUnlocked(true);setNativeMessage('Protection biométrique locale désactivée.');return}
    const [hardware,enrolled]=await Promise.all([LocalAuthentication.hasHardwareAsync(),LocalAuthentication.isEnrolledAsync()]);
    if(!hardware||!enrolled){setNativeMessage('Configurez Face ID, Touch ID ou une empreinte sur le téléphone avant d’activer ce verrou.');return}
    const result=await LocalAuthentication.authenticateAsync({promptMessage:'Activer la protection biométrique SINJIRA',cancelLabel:'Annuler',disableDeviceFallback:false});
    if(!result.success)return;
    await SecureStore.setItemAsync(BIOMETRIC_LOCK_STORAGE,'1');setBiometricEnabled(true);setIsUnlocked(true);setNativeMessage('Protection biométrique locale activée.');
  };
  ...
  return <SafeAreaView style={styles.root}>
    ...
    {nativeMessage?<View style={styles.nativeNotice}><Text style={styles.nativeNoticeText}>{nativeMessage}</Text></View>:null}
    ...
    <WebView key={webViewKey} ref={webViewRef} source={{uri:currentUrl}} style={styles.webview} originWhitelist={['https://*','sinjira://*']} sharedCookiesEnabled thirdPartyCookiesEnabled={false} domStorageEnabled javaScriptEnabled cacheEnabled injectedJavaScriptBeforeContentLoaded={injectedSecurityScript} startInLoadingState pullToRefreshEnabled={Platform.OS==='ios'} allowsBackForwardNavigationGestures={Platform.OS==='ios'} onNavigationStateChange={onNavigationStateChange} onShouldStartLoadWithRequest={shouldStart} renderLoading={()=><View style={styles.loading}><ActivityIndicator size="large"/><Text style={styles.loadingText}>Chargement de SINJIRA…</Text></View>} renderError={()=><View style={styles.errorBox}><Text style={styles.errorTitle}>Connexion indisponible</Text><Text style={styles.errorText}>Vérifiez votre connexion Internet, puis touchez Réessayer.</Text><Pressable accessibilityRole="button" onPress={()=>setWebViewKey(v=>v+1)} style={styles.retryButton}><Text style={styles.retryText}>Réessayer</Text></Pressable></View>}/>
...
const styles=StyleSheet.create({
  root:{flex:1,backgroundColor:'#070914'},topbar:{minHeight:58,paddingHorizontal:10,paddingVertical:8,flexDirection:'row',alignItems:'center',justifyContent:'space-between',backgroundColor:'#0d1020',borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:'#2a3047'},topActions:{flexDirection:'row',gap:5,alignItems:'center'},brand:{color:'#ffffff',fontSize:20,fontWeight:'800',letterSpacing:.4},subtitle:{color:'#9da8c7',fontSize:11,marginTop:1},refreshButton:{borderWidth:1,borderColor:'#384260',borderRadius:999,paddingHorizontal:8,paddingVertical:7},refreshText:{color:'#e9edff',fontWeight:'700',fontSize:10},nativeNotice:{paddingHorizontal:12,paddingVertical:7,backgroundColor:'#12172a'},nativeNoticeText:{color:'#c8d0eb',fontSize:11,textAlign:'center'},quickRail:{flexDirection:'row',gap:6,paddingHorizontal:8,paddingVertical:8,backgroundColor:'#090c17'},quickButton:{flex:1,minHeight:34,alignItems:'center',justifyContent:'center',borderRadius:10,borderWidth:1,borderColor:'#252d45',backgroundColor:'#11162a'},quickText:{color:'#dce3ff',fontSize:10,fontWeight:'700',textAlign:'center'},webview:{flex:1,backgroundColor:'#070914'},loading:{...StyleSheet.absoluteFill,alignItems:'center',justifyContent:'center',gap:12,backgroundColor:'#070914'},loadingText:{color:'#c8d0eb',fontSize:14},lockScreen:{flex:1,alignItems:'center',justifyContent:'center',padding:30,backgroundColor:'#070914'},lockTitle:{color:'#ffffff',fontSize:22,fontWeight:'800',marginBottom:10,textAlign:'center'},lockText:{color:'#aab3cf',textAlign:'center',lineHeight:21,marginBottom:14},lockNote:{color:'#c8d0eb',textAlign:'center',fontSize:12,marginBottom:16},errorBox:{flex:1,alignItems:'center',justifyContent:'center',padding:28,backgroundColor:'#070914'},errorTitle:{color:'#ffffff',fontSize:19,fontWeight:'800',marginBottom:8},errorText:{color:'#aab3cf',textAlign:'center',lineHeight:20,marginBottom:18},retryButton:{borderRadius:10,paddingHorizontal:18,paddingVertical:10,backgroundColor:'#e4e9ff'},retryText:{color:'#10152a',fontWeight:'800'},bottomNav:{minHeight:58,flexDirection:'row',paddingHorizontal:6,paddingTop:5,paddingBottom:Platform.OS==='android'?6:3,backgroundColor:'#0d1020',borderTopWidth:StyleSheet.hairlineWidth,borderTopColor:'#2a3047'},tab:{flex:1,alignItems:'center',justifyContent:'center',borderRadius:10,marginHorizontal:2},tabSelected:{backgroundColor:'#1b2340'},tabText:{color:'#8e98b7',fontSize:11,fontWeight:'700'},tabTextSelected:{color:'#ffffff'}
});
