...
const employmentDestinations = [
  {
    label: 'Mon espace Emploi',
    description: 'Ouvrir le profil professionnel et le suivi de candidatures dans la surface Web privée existante.',
    path: '/compte/emploi.html?surface=web',
  },
  {
    label: 'Vie privée',
    description: 'Consulter les choix de confidentialité du compte sans mélanger les données Emploi avec vos autres espaces.',
    path: '/compte/vie-privee.html?surface=web',
  },
  {
    label: 'Ma sécurité',
    description: 'Ouvrir le Centre de sécurité. Les renseignements de sécurité ne servent pas à classer une candidature.',
    path: '/compte/securite.html',
  },
  {
    label: 'Paramètres',
    description: 'Accéder aux réglages généraux du compte dans leur surface protégée existante.',
...
export function NativeEmploymentHub({ onOpenPath, onBack }: Props) {
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      ...
      <View style={styles.hero}>
        ...
        <Text style={styles.intro}>
          Ce hub natif ne lit aucun profil professionnel, aucune candidature, aucune note privée, aucun statut ni aucune date. Il sert uniquement à vous orienter vers l’espace Emploi SINJIRA existant.
        </Text>
      </View>

      <View style={styles.boundaryCard}>
        <Text style={styles.cardKicker}>PROTÉGER SANS SURVEILLER</Text>
        <Text style={styles.boundaryTitle}>Vos autres espaces restent séparés</Text>
        <Text style={styles.boundaryText}>
          Le natif ne récupère rien depuis le Registre personnel, Rencontres, Histoire de vie, vos relations, votre localisation de sécurité ou le Mode Voyage pour enrichir ou classer une candidature.
        </Text>
      </View>

      <Text style={styles.sectionTitle}>Choisir une destination</Text>
      <Text style={styles.sectionText}>
        Le natif ne crée, ne modifie, ne supprime et ne classe aucune candidature.
      </Text>
      <View style={styles.destinationList}>
        {employmentDestinations.map((item) => (
          <DestinationCard
            key={item.path}
            label={item.label}
      ...
      <View style={styles.privacyNote}>
        <Text style={styles.privacyTitle}>Aucun formulaire Emploi n’est reproduit dans le natif</Text>
        <Text style={styles.privacyText}>
          Aucun titre professionnel, résumé, compétence, préférence de travail, employeur, poste, lien source, prochaine étape ou note privée n’est conservé par cet écran React Native. La source de vérité reste la surface Web protégée.
        </Text>
      </View>
    </ScrollView>
  );
