...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(!pepper)throw new Error('Configuration licence manquante');
    const body=await req.json().catch(()=>({}));

    if(body?.action==='health'){
      return json({ok:true,service:'redeem-license-code',version:'24.4.3',pepper_configured:true});
    }
    ...
    return json({ok:false,error:'Activation impossible.'},500);
  }
});
