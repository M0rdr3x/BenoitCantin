...
async function audit(s:any,userId:string,action:string,r:any,metadata:Record<string,unknown>={}){
  try{
    await s.from('admin_audit_log').insert({
      ...
      summary:`${r?.network||'social'}:${r?.target_type||'target'}`,
      metadata:{network:r?.network||null,target_type:r?.target_type||null,target_id:r?.target_id||null,...metadata}
    });
  }catch(e){console.warn('[SINJIRA social audit]',e)}
}

async function authoritativeTargetUser(s:any,r:any){
  const network=String(r?.network||''),type=String(r?.target_type||''),id=r?.target_id;
  if(!id)return null;
  ...
  const {data}=await s.from(table).select(column).eq('id',id).maybeSingle();
  return data?.[column]||null;
}

function suspensionDays(value:unknown){
  const n=Number(value);
  if(!Number.isFinite(n)||!Number.isInteger(n))throw new Error('INVALID_SUSPENSION_DAYS');
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(a==='resolve_report'){
      const {data:r,error:readError}=await s.from('social_reports').select('*').eq('id',b.report_id).single();
      if(readError)throw readError;
      const {error}=await s.from('social_reports').update({status:'resolved',reviewed_at:new Date().toISOString(),reviewed_by:user.id}).eq('id',r.id);
      if(error)throw error;
      await audit(s,user.id,'resolve_social_report',r);
      return json({ok:true});
    }
    ...
    if(a==='remove_reported_content'){
      ...
      if(error)throw error;
      const map:any={
        real:{post:'social_real_posts',comment:'social_real_comments',message:'social_real_messages'},
        character:{post:'social_character_posts',comment:'social_character_comments',message:'social_character_messages'}
      };
      const table=map?.[r.network]?.[r.target_type];
      if(!table)throw new Error('UNSUPPORTED_MODERATION_TARGET');
      const {error:deleteError}=await s.from(table).delete().eq('id',r.target_id);
      if(deleteError)throw deleteError;
      const {error:resolveError}=await s.from('social_reports').update({status:'resolved',reviewed_at:new Date().toISOString(),reviewed_by:user.id}).eq('id',r.id);
      if(resolveError)throw resolveError;
      await audit(s,user.id,'remove_reported_content',r);
      return json({ok:true});
    }
    ...
    if(a==='suspend_reported_user'){
      const {data:r,error}=await s.from('social_reports').select('*').eq('id',b.report_id).single();
      if(error)throw error;
      const uid=await authoritativeTargetUser(s,r);
      if(!uid)throw new Error('TARGET_USER_NOT_FOUND');
      if(uid===user.id)throw new Error('CANNOT_SUSPEND_SELF');
      const days=suspensionDays(b.days??7),until=new Date(Date.now()+days*86400000).toISOString();
      const {error:suspensionError}=await s.from('social_suspensions').upsert({
        user_id:uid,
        reason:`Signalement ${r.id}: ${String(r.reason||'').slice(0,500)}`,
        until_at:until,
        created_by:user.id
      },{onConflict:'user_id'});
      if(suspensionError)throw suspensionError;
      await audit(s,user.id,'suspend_reported_user',r,{suspended_user_id:uid,days,until_at:until});
