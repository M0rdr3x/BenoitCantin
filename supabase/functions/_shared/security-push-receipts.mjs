...
export function resolveSecurityPushReceipts(pendingRows, receiptData) {
  ...
  for (const row of pendingRows) {
    ...
    const endpointId = typeof row?.endpoint_id === 'string' ? row.endpoint_id.trim() : '';
    if (!endpointId) throw new TypeError('INVALID_PUSH_ENDPOINT_ID');
    ...
    if (classifySecurityPushReceipt(data[receiptId]) === 'device_not_registered') {
      invalidEndpointIds.push(endpointId);
    }
  }
