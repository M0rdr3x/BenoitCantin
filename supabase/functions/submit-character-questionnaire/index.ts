...
function participantSummary(src:Record<string,unknown>){
 const omitted=new Set(['compte_courriel','compte_pseudo']);
 const rows=Object.entries(src||{}).filter(([k,v])=>!omitted.has(k)&&!k.startsWith('photo')&&v!==''&&v!=null).map(([k,v])=>{
 ...
 return rows.length?`<table style="border-collapse:collapse;width:100%;max-width:760px">${rows.join('')}</table>`:'<p>Aucune réponse textuelle à afficher.</p>';
}
...
async function notify(service:any,sub:any,user:any,p:any,answers:Record<string,unknown>,updated=false){
 ...
 if(user.email){
  try{const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${resend}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[user.email],subject:updated?'SINJIRA™ — copie de votre questionnaire mis à jour':'SINJIRA™ — copie de votre participation au Registre',html:`<h2>Votre participation est enregistrée</h2><p>Bonjour ${escapeHtml(p?.pseudo||p?.display_name||'membre SINJIRA™')},</p><p>Votre questionnaire du Registre des Consciences a bien été ${updated?'mis à jour':'enregistré'} dans SINJIRA™.</p><p><strong>Numéro de dossier :</strong> ${escapeHtml(sub.id)}</p><p>La photographie éventuelle n’est volontairement pas jointe à ce courriel. Votre copie textuelle est ci-dessous.</p>${participantSummary(answers)}<p style="margin-top:24px">Votre personnage restera provisoire jusqu’à validation par Benoit Cantin. Les renseignements sources suivent les règles de conservation et de suppression indiquées dans le Registre.</p>`})});participantEmail=r.ok;if(!r.ok)console.warn('participant resend',await r.text())}catch(e){console.warn('participant email failed',e)}
 }
