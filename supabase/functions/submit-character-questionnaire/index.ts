...
Deno.serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:corsHeaders});if(req.method!=='POST')return json({ok:false,error:'Méthode non autorisée.'},405);
 try{const user=await requiredUser(req),service=serviceClient(),{answers,photo_path,manual_only=true}=await req.json();const [{data:p},{data:existingSubmission},{data:existingCharacter}]=await Promise.all([service.from('profiles').select('pseudo,display_name').eq('user_id',user.id).maybeSingle(),service.from('character_submissions').select('id,status').eq('user_id',user.id).limit(1).maybeSingle(),service.from('characters').select('id,status').eq('user_id',user.id).limit(1).maybeSingle()]);if(existingSubmission||existingCharacter)return json({ok:false,error:'Ce Compte SINJIRA possède déjà une demande ou son unique personnage. Le Registre est verrouillé pour ce compte.',code:'ONE_CHARACTER_PER_ACCOUNT'},409);
