...
function ownedPhotoPath(userId:string,value:unknown){
  ...
  if(path.length>500||!path.startsWith(`${userId}/`)||path.includes('..')||/[\u0000-\u001f\\]/.test(path)||!/^[-a-zA-Z0-9_./]+\.(?:jpe?g|png|webp|avif|gif)$/i.test(path))throw new Error('INVALID_PHOTO_PATH');
  return path;
}
...
async function generate(answers:Record<string,unknown>,service:any){
  ...
  if(!res.ok)throw new Error(`OPENAI_${res.status}`);const data=await res.json();
  try{return {model,bible:JSON.parse(data.output_text)}}catch{throw new Error('OPENAI_INVALID_OUTPUT')}
}
async function notifyAdmin(service:any,sub:any,user:any,p:any,updated=false){
  let internal=false,email=false;
  try{
    const {error}=await service.from('admin_notifications').insert({notification_type:'character_submission',title:updated?'Questionnaire SINJIRA™ mis à jour':'Nouveau questionnaire SINJIRA™',body:`${updated?'Mise à jour':'Nouvelle participation'} de ${p?.pseudo||p?.display_name||user.email||'un membre'}.`,related_user_id:user.id,related_entity_type:'character_submission',related_entity_id:sub.id});
    internal=!error;
  }catch(e){console.warn('admin notification unavailable',e)}
  const resend=Deno.env.get('RESEND_API_KEY'),from=Deno.env.get('REPORT_FROM_EMAIL'),to=Deno.env.get('CHARACTER_REPORT_TO_EMAIL')||Deno.env.get('FRACTURE_REPORT_TO_EMAIL')||'kingtyrano@gmail.com';
  if(resend&&from){
    try{
      const r=await fetch('https://api.resend.com/emails',{method:'POST',signal:AbortSignal.timeout(10_000),headers:{Authorization:`Bearer ${resend}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[to],subject:updated?'SINJIRA™ — questionnaire personnage mis à jour':'SINJIRA™ — nouveau questionnaire personnage',html:`<p>${updated?'Un questionnaire':'Une nouvelle participation au Registre des Consciences'} a été ${updated?'mis à jour':'enregistrée'}.</p><p><strong>Compte :</strong> ${String(p?.pseudo||p?.display_name||'Compte SINJIRA™').replace(/[<>&]/g,'')}<br><strong>Courriel :</strong> ${String(user.email||'').replace(/[<>&]/g,'')}<br><strong>Dossier :</strong> ${sub.id}</p><p>Consultez l’administration SINJIRA™ pour voir le questionnaire complet.</p>`})});
      email=r.ok;if(!r.ok)console.warn('resend',r.status);
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const body=await req.json().catch(()=>{throw new Error('INVALID_JSON')});
    const answers=validateAnswers(body?.answers||{}),photo_path=ownedPhotoPath(user.id,body?.photo_path),update_existing=body?.update_existing===true;
    const aiRequested=body?.manual_only===false,aiAllowed=aiRequested&&characterAiEnabled(),owner=String(user?.email||'').trim().toLowerCase()==='kingtyrano@gmail.com';
    const [profileRes,submissionRes,characterRes]=await Promise.all([
    ...
    const lookupError=profileRes.error||submissionRes.error||characterRes.error;if(lookupError)throw lookupError;
    const p=profileRes.data,existingSubmission=submissionRes.data,existingCharacter=characterRes.data;
    ...
    if(owner&&update_existing&&(existingSubmission||existingCharacter)){
      ...
      if(existingSubmission){
        ...
        const {data:updated,error:updateError}=await service.from('character_submissions').update({account_pseudo:p?.pseudo||p?.display_name||'AbyssTime',account_email:user.email||'',source_payload:answers,photo_path:photo_path||existingSubmission.photo_path||null,status:nextStatus,source_purged_at:null}).eq('id',existingSubmission.id).select('*').single();
        if(updateError)throw updateError;sub=updated;
      }else{
        const {data:created,error:createError}=await service.from('character_submissions').insert({user_id:user.id,account_pseudo:p?.pseudo||p?.display_name||'AbyssTime',account_email:user.email||'',source_payload:answers,photo_path:photo_path||null,status:existingCharacter?'assigned':'submitted'}).select('*').single();
        if(createError)throw createError;sub=created;
      }
    ...
    if(aiAllowed){
      ...
      catch(e){const {error:runError}=await service.from('character_generation_runs').insert({submission_id:sub.id,status:'failed',error_text:String(e?.message||e).slice(0,1000)});if(runError)console.warn('[SINJIRA character generation run]',runError.message)}
    }
    if(generated){
    ...
    if(e?.message==='INVALID_JSON'||e?.message==='INVALID_ANSWERS')return json({ok:false,error:'Questionnaire invalide.'},400);
    if(e?.message==='ANSWERS_TOO_LARGE')return json({ok:false,error:'Le questionnaire dépasse la taille autorisée.'},413);
    if(e?.message==='INVALID_PHOTO_PATH')return json({ok:false,error:'Le chemin de photo ne correspond pas au stockage privé de votre compte.'},400);
    return json({ok:false,error:'Impossible de transmettre le questionnaire.'},500);
  }
});
