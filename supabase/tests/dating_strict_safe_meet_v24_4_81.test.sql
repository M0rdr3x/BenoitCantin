...
select ok(position('private.sinjira_points_spend(v_payer_user,1' in replace(pg_get_functiondef('public.dating_safe_meet_opt_in(uuid,text[],text)'::regprocedure),' ',''))>0,'exactement 1 Point SINJIRA est débité à la génération');
select ok(position($q$'dating_safe_meet'$q$ in replace(pg_get_functiondef('public.dating_safe_meet_opt_in(uuid,text[],text)'::regprocedure),' ',''))>0,'le débit est rattaché au module Rencontres dans le ledger universel');
