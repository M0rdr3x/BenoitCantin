...
select ok(exists(select 1 from pg_policies where schemaname='public' and tablename='social_live_rooms' and policyname='social_live_rooms_read' and qual ilike '%visibility%' and qual ilike '%social_live_room_members%'),'salon privé exige une adhésion');
