...
select ok(position('SINJIRA_MINIMUM_AGE_11' in pg_get_functiondef('sinjira_profile_internal.private_profile_save(date,text,text[],text,text,text,text,text,text,text,date,text)'::regprocedure))>0 and position('YOUTH_JURISDICTION_NOT_ENABLED' in pg_get_functiondef('sinjira_profile_internal.private_profile_save(date,text,text[],text,text,text,text,text,text,text,date,text)'::regprocedure))>0 and position('GUARDIAN_AUTHORIZATION_REQUIRED_UNDER_14' in pg_get_functiondef('sinjira_profile_internal.private_profile_save(date,text,text[],text,text,text,text,text,text,text,date,text)'::regprocedure))>0,'garde âge 11+, juridiction et tuteur conservée');
