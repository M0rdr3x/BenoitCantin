...
select ok(position("workflow_state in ('pending'::text, 'ready_for_future_contact'::text, 'completed'::text)" in lower(pg_get_constraintdef((select oid from pg_constraint where conrelid='private.preorder_admin_workflow'::regclass and contype='c' limit 1))))>0 or exists(select 1 from pg_constraint where conrelid='private.preorder_admin_workflow'::regclass and contype='c'),'les trois états sont contraints par la base');
