...
select ok(position('auth.uid()' in pg_get_functiondef('preorder_user_internal.product_preorder_reserve_confirmed(text,text,integer,boolean,text,boolean)'::regprocedure))>0,'implémentation vérifie auth.uid()');
select ok(position('preorder-disclosure-v24.5.31' in pg_get_functiondef('preorder_user_internal.product_preorder_reserve_confirmed(text,text,integer,boolean,text,boolean)'::regprocedure))>0,'version exacte V24.5.31 exigée');
select ok(position('p_disclosure_acknowledged IS DISTINCT FROM true' in pg_get_functiondef('preorder_user_internal.product_preorder_reserve_confirmed(text,text,integer,boolean,text,boolean)'::regprocedure))>0,'confirmation explicite true exigée');
select ok(position('financial_commitment = false' in pg_get_functiondef('preorder_user_internal.product_preorder_reserve_confirmed(text,text,integer,boolean,text,boolean)'::regprocedure))>0 and position($q$payment_status = 'not_collected'$q$ in pg_get_functiondef('preorder_user_internal.product_preorder_reserve_confirmed(text,text,integer,boolean,text,boolean)'::regprocedure))>0,'réservation reste sans paiement ni engagement financier');
select ok(position('PREORDER_DISCLOSURE_REQUIRED' in pg_get_functiondef('preorder_user_internal.product_preorder_reserve(text,text,integer,boolean)'::regprocedure))>0 and position('insert into public.product_preorders' in lower(pg_get_functiondef('preorder_user_internal.product_preorder_reserve(text,text,integer,boolean)'::regprocedure)))=0,'ancien chemin ne peut plus écrire sans accusé');
