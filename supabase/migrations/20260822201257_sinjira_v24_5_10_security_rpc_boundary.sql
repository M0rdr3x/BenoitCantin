...
comment on schema sinjira_security_internal is
  'Implémentations privilégiées des RPC du centre de sécurité SINJIRA. Hors schéma API public; wrappers public SECURITY INVOKER seulement.';
...
begin
  ...
  if v_count <> 17 then
    raise exception 'V24.5.10 attend 17 RPC sécurité SECURITY DEFINER publiques; trouvé %', v_count;
  end if;
  ...
  loop
    ...
    execute format('alter function public.%I(%s) set schema sinjira_security_internal', r.proname, r.arg_types);

    execute format('revoke all on function sinjira_security_internal.%I(%s) from public, anon', r.proname, r.arg_types);
    execute format('grant execute on function sinjira_security_internal.%I(%s) to authenticated, service_role', r.proname, r.arg_types);

    if r.result_type like 'TABLE(%' then
      v_body := format('select * from sinjira_security_internal.%I(%s)', r.proname, v_call_args);
    else
      v_body := format('select sinjira_security_internal.%I(%s)', r.proname, v_call_args);
    end if;
