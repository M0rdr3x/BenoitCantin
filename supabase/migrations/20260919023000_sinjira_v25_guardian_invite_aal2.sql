...
-- Pour conserver 16 nibbles réellement aléatoires, la génération saute les nibbles
-- UUID v4 réservés à la version et au variant au lieu de tronquer un UUID brut.
...
alter table public.guardian_signup_invites
  ...
  check (invite_code ~ '^YOUTH-[A-Z0-9]{10}([A-Z0-9]{6})?$');
...
begin
  ...
  loop
    -- UUID v4 contient deux nibbles réservés : version (position 13) et variant
    -- (position 17) dans la chaîne hexadécimale sans tirets. On les saute pour
    -- obtenir 16 nibbles réellement aléatoires.
...
comment on function public.create_guardian_signup_invite() is
  'V25: émission code parental à usage unique de 16 caractères réservée à un adulte en session AAL2; 16 nibbles aléatoires hors métadonnées UUID v4; les anciens codes 10 caractères restent consommables jusqu à expiration.';
