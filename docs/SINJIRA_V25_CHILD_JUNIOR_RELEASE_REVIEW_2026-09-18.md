...
- snapshot release : **39 migrations futures non revues**, empreintes intactes, reviewed batch et ledger inchangés.

La classification exhaustive des workflows rouges de ce HEAD montre que leurs étapes métier/sécurité passent avant de s'arrêter sur le **ledger production volontairement bloqué** par les 39 migrations non revues. Le prévol Supabase suit la même logique : ses tests de sécurité passent, puis la vérification du dépôt s'arrête sur ce ledger.
...
### Extensions privées achetées

Le schéma `extensions` ne possédait auparavant qu'un indicateur `is_public`; il n'existait aucun moyen serveur d'exprimer « extension privée achetée ».

La migration forward-only :

`20260922030000_sinjira_v25_extension_product_access.sql`

ajoute un `product_slug` optionnel, sans convertir les extensions existantes. Une extension publique reste gratuite. Une extension privée liée à un produit devient visible au membre standard uniquement si le droit produit canonique confirme un entitlement réel ou une commande `paid`. Les commandes `pending` restent insuffisantes. Les comptes créateur/famille 13+ gardent leur catalogue complet et les 11–12 famille restent limités à une fiche protégée non ouvrable.

Le pgTAP famille est porté à **54 assertions** avec quatre preuves supplémentaires : extension payante invisible avant achat en RLS + RPC, puis visible après commande `paid` en RLS + RPC, sans entitlement artificiel.

Cette trente-neuvième migration reste **non revue production**.
...
Cette réouverture de revue **ne transforme aucune migration en migration production revue**. Les 39 migrations restent non revues au sens du `production-reviewed-migration-batch.txt`.

## 3. Lot local futur actuellement non revu

Le snapshot de revue attend exactement **39 migrations locales futures non revues**.
...
2. Relire les **39 migrations** dans l’ordre chronologique canonique; la migration `20260921005000` appartient fonctionnellement au lot Mode Voyage mais reste la dernière par timestamp.
