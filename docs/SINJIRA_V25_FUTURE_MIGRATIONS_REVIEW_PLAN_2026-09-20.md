...
Relecture technique rafraîchie sur le HEAD fonctionnel `92f74a8bb1fc31452b08b67ca27a67ec9008acab`, sans modifier le statut des cases B1–B5 :
...
- la preuve locale d'inscription reconstruit la base puis passe **74/74** assertions `child_guardian_signup_v25.test.sql`, **11/11** assertions du coffre privé enfant et un vrai parcours Auth HTTP à exactement 11 ans; les nouvelles assertions prouvent aussi le refus d'un second code parental après rétablissement, son maintien inutilisé, l'absence de second lien `verified`, l'absence de privilège `UPDATE` navigateur sur `guardian_links` et la disparition de l'ancienne policy `guardian_guardian_update`; le navigateur 11 ans reste couvert séparément;
...
- la portée technique est confirmée fail-closed au Canada pour les 11–17 ans (`YOUTH_JURISDICTION_NOT_ENABLED` ailleurs); toute décision d'élargissement à une autre juridiction reste une décision humaine séparée;
