...
async function ensureOwnRows(){
  ...
  if(!profileResult.data){
    const {error}=await s.from('dating_profiles').insert({user_id:user.id});
    if(error)throw error;
  }
  if(!prefResult.data){
    const {error}=await s.from('dating_preferences').insert({user_id:user.id});
    if(error)throw error;
  }
}
...
async function loadOwnForm(){
  ...
  form.elements.intro.value=p.intro||'';
  form.elements.min_age.value=d.min_age||18;
  form.elements.max_age.value=d.max_age||99;
  ...
  form.elements.partner_description.value=d.partner_description||'';
  form.elements.dealbreakers.value=d.dealbreakers||'';
  form.elements.use_registry_answers.checked=!!p.use_registry_answers;
  const seeking=new Set(d.seeking_genders||[]);
  form.querySelectorAll('input[name="seeking_genders"]').forEach(node=>{node.checked=seeking.has(node.value)});
}

async function saveProfile({silent=false}={}){
  const minAge=Number(form.elements.min_age.value||18);
  const maxAge=Number(form.elements.max_age.value||99);
  if(minAge>maxAge)throw new Error('AGE_RANGE');
  const seeking=checkedValues('seeking_genders');
  const profilePayload={
    user_id:user.id,
    gender_identity:form.elements.gender_identity.value||null,
    region:String(form.elements.region.value||'').trim(),
    intro:String(form.elements.intro.value||'').trim(),
    values_tags:splitTags(form.elements.values_tags.value),
    interests_tags:splitTags(form.elements.interests_tags.value),
    lifestyle_tags:splitTags(form.elements.lifestyle_tags.value),
    communication_tags:splitTags(form.elements.communication_tags.value),
    goals_tags:splitTags(form.elements.goals_tags.value),
    use_registry_answers:!!form.elements.use_registry_answers.checked
  };
  const preferencePayload={
    user_id:user.id,min_age:minAge,max_age:maxAge,seeking_genders:seeking,
    wanted_values:splitTags(form.elements.wanted_values.value),
    wanted_interests:splitTags(form.elements.wanted_interests.value),
    wanted_lifestyle:splitTags(form.elements.wanted_lifestyle.value),
    wanted_communication:splitTags(form.elements.wanted_communication.value),
    wanted_goals:splitTags(form.elements.wanted_goals.value),
    partner_description:String(form.elements.partner_description.value||'').trim(),
    dealbreakers:String(form.elements.dealbreakers.value||'').trim()
  };
  const [a,b]=await Promise.all([
    s.from('dating_profiles').upsert(profilePayload,{onConflict:'user_id'}),
    s.from('dating_preferences').upsert(preferencePayload,{onConflict:'user_id'})
  ]);
...
async function loadSelfStatus(){
  const {data,error}=await s.rpc('dating_self_status');
  if(error)throw error;
  const state=data||{};
  document.querySelectorAll('[data-dating-adult-zone]').forEach(node=>{node.hidden=state.adult===false});
  if(state.adult===false){
    if(statusLabel)statusLabel.textContent='Accès refusé · 18+ uniquement';
    setStatus(statusNode,'Les rencontres SINJIRA™ sont strictement réservées aux adultes.','error');
    return state;
  }
  if(statusLabel){
    statusLabel.textContent=state.eligible
      ?`Profil actif · ${state.age||'18+'} ans · célibataire confirmé`
      :state.enabled?'Profil à reconfirmer ou incomplet':'Profil non actif';
  }
  return state;
}
...
function candidateCard(item){
  ...
  return `<article class="account-card" data-dating-candidate="${escapeHtml(item.profile_id)}">
    <div class="account-welcome-strip"><strong>${escapeHtml(item.blind_alias)}</strong><span>${escapeHtml(String(item.compatibility_score))}% compatible</span></div>
    <h3>${escapeHtml(item.age_band)} · ${escapeHtml(item.region||'Région non précisée')}</h3>
    <p>${escapeHtml(item.intro||'Présentation à découvrir par la conversation.')}</p>
    <ul>${reasons||'<li>Compatibilité globale selon vos critères.</li>'}</ul>
    <div class="hero-actions"><button class="btn btn-primary btn-small" type="button" data-dating-request>Proposer une discussion</button></div>
  </article>`;
}

async function loadCandidates(state){
  if(!candidatesBox)return;
  if(!state?.eligible){candidatesBox.innerHTML='<p>Activez et confirmez votre profil pour recevoir des propositions anonymes.</p>';return;}
  const {data=[],error}=await s.rpc('dating_compatibility_candidates',{p_limit:8});
  if(error)throw error;
  ...
  candidatesBox.querySelectorAll('[data-dating-candidate]').forEach(card=>{
    card.querySelector('[data-dating-request]')?.addEventListener('click',async()=>{
      const button=card.querySelector('[data-dating-request]');
      button.disabled=true;
      const {error:requestError}=await s.rpc('dating_request_conversation',{p_candidate_profile_id:card.dataset.datingCandidate});
      if(requestError){button.disabled=false;setStatus(statusNode,explainError(requestError,'Impossible de proposer cette discussion.'),'error');return;}
      setStatus(statusNode,'Proposition de discussion envoyée sans dévoiler votre identité.','success');
      await refreshAll();
    });
  });
}
...
function connectionCard(item){
  ...
  const identity=item.identity_revealed
  ...
  return `<article class="account-card" data-dating-connection="${escapeHtml(item.connection_id)}">
    ...
    <p>${escapeHtml(item.intro||'')}</p><p><small>${escapeHtml(item.region||'Région non précisée')} · Vous : ${Number(item.my_message_count||0)}/10 · Autre personne : ${Number(item.their_message_count||0)}/10 avant dévoilement</small></p>
...
async function loadConnections(){
  ...
  connectionsBox.querySelectorAll('[data-dating-connection]').forEach(card=>{
    ...
    card.querySelector('[data-dating-reveal]')?.addEventListener('click',async()=>{
      ...
      setStatus(statusNode,'Consentement au dévoilement enregistré. Le profil restera caché tant que l’autre personne n’aura pas aussi accepté.','success');
...
async function openConversation(id){
  activeConnection=id;
  const info=lastOverview.find(row=>row.connection_id===id);
  if(conversationTitle)conversationTitle.textContent=info?.identity_revealed?`Discussion avec ${info.revealed_name}`:`Discussion avec ${info?.blind_alias||'un profil compatible'}`;
  const {data=[],error}=await s.rpc('dating_conversation',{p_connection_id:id});
  if(error){setStatus(statusNode,explainError(error,'Impossible de charger cette conversation.'),'error');return;}
  ...
  conversationZone.hidden=false;
  conversationZone.scrollIntoView({behavior:'smooth',block:'start'});
}

async function refreshAll(){
  const state=await loadSelfStatus();
  if(state.adult!==false){await Promise.all([loadCandidates(state),loadConnections()]);}
}

form?.addEventListener('submit',async event=>{
  event.preventDefault();
  try{await saveProfile();await refreshAll()}catch(error){setStatus(statusNode,error?.message==='AGE_RANGE'?'La plage d’âge recherchée est invalide.':explainError(error,'Impossible d’enregistrer ce profil.'),'error')}
});

document.querySelector('[data-dating-confirm]')?.addEventListener('click',async()=>{
  try{
    await saveProfile({silent:true});
    const {error}=await s.rpc('dating_confirm_single_and_serious');
    if(error)throw error;
    setStatus(statusNode,'Profil activé : vous confirmez être majeur, célibataire, disponible et à la recherche d’une relation sérieuse.','success');
    await refreshAll();
  }catch(error){setStatus(statusNode,explainError(error,'Impossible d’activer le profil.'),'error')}
});

document.querySelector('[data-dating-pause]')?.addEventListener('click',async()=>{
  const {error}=await s.rpc('dating_pause_profile',{});
  if(error){setStatus(statusNode,explainError(error,'Impossible de mettre le profil en pause.'),'error');return;}
  setStatus(statusNode,'Profil de rencontres en pause. Vous ne recevez plus de nouvelles propositions.','success');
  await refreshAll();
});

document.querySelector('[data-dating-partnered]')?.addEventListener('click',async()=>{
  if(!confirm('Confirmer que vous êtes maintenant en couple ou indisponible? Les discussions de rencontres actives seront fermées.'))return;
  const {error}=await s.rpc('dating_pause_profile',{p_relationship_status:'partnered'});
  if(error){setStatus(statusNode,explainError(error,'Impossible de mettre à jour votre disponibilité.'),'error');return;}
  setStatus(statusNode,'Votre profil de rencontres est désactivé et vos discussions de rencontres actives sont fermées.','success');
  await refreshAll();
});

document.querySelector('[data-dating-import-registry]')?.addEventListener('click',async()=>{
  try{
    form.elements.use_registry_answers.checked=true;
    await saveProfile({silent:true});
    const {data,error}=await s.rpc('dating_import_registry_traits');
    if(error)throw error;
    setStatus(statusNode,`${Array.isArray(data)?data.length:0} repère(s) non sensible(s) du Registre importé(s).`,'success');
    await refreshAll();
  }catch(error){setStatus(statusNode,explainError(error,'Impossible d’importer les repères du Registre.'),'error')}
});
...
messageForm?.addEventListener('submit',async event=>{
  ...
  try{
    const {error}=await s.rpc('dating_send_message',{p_connection_id:activeConnection,p_body:body});
    if(error)throw error;
    messageForm.reset();
    await Promise.all([openConversation(activeConnection),loadConnections()]);
  }catch(error){setStatus(statusNode,explainError(error,'Impossible d’envoyer ce message.'),'error')}
  finally{if(button)button.disabled=false}
});

(async()=>{
  try{
    user=await requireCommunityUser(location.pathname+location.search);
    await ensureOwnRows();
    await loadOwnForm();
    await refreshAll();
  }catch(error){
    if(error?.message!=='RULES_REQUIRED'&&error?.message!=='Connexion requise')setStatus(statusNode,explainError(error,'Le module Rencontres n’a pas pu être chargé.'),'error');
  }
})();
