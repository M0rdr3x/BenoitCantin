...
function setField(name,value){
  ...
  if(input && !('length' in input && input.tagName===undefined)) input.value=value??'';
}
...
function profilePayload(activeOverride=null){
  return {
    ...
    values_sought:checkedValues('values_sought'),
...
async function loadEligibility(){
  const {data,error}=await getSupabase().rpc('dating_my_eligibility');
  if(error)throw error;
  eligibility=data||{eligible:false,reason:'unknown'};
  renderEligibility(eligibility);
  return eligibility;
}

async function loadOwnProfile(){
  const {data,error}=await getSupabase().from('dating_profiles').select('*').eq('user_id',user.id).maybeSingle();
  if(error)throw error;
  fillProfile(data);
}
...
function listMarkup(items,emptyLabel){
  ...
  return list.length?`<ul>${list.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:`<p class="dating-help">${escapeHtml(emptyLabel)}</p>`;
}

async function requestIntroduction(targetUserId,button){
  button.disabled=true;
  try{
    const {error}=await getSupabase().rpc('dating_request_introduction',{p_target_user_id:targetUserId});
    if(error)throw error;
    socialStatus(statusNode,'Présentation demandée. L’autre personne peut accepter ou refuser sans pression.','success');
...
async function loadRecommendations(){
  if(!eligibility?.eligible||!ownProfile?.active){
    recommendationsNode.innerHTML='<div class="dating-empty">Activez votre profil de rencontre pour recevoir des propositions compatibles.</div>';
    return;
  }
  const {data,error}=await getSupabase().rpc('dating_recommendations',{p_limit:8});
  if(error)throw error;
  const rows=Array.isArray(data)?data:[];
  recommendationsNode.innerHTML=rows.map(row=>`<article class="dating-candidate" data-dating-candidate="${escapeHtml(row.user_id)}"><div class="dating-candidate-head"><div><span class="dating-badge">Présentation privée</span><h3>${escapeHtml(row.pseudo||'Membre SINJIRA')}</h3><p class="dating-help">Aucune photo n’est utilisée dans cette proposition.</p></div><div class="dating-score"><strong>${Number(row.compatibility_score)||0}%</strong><span>compatibilité</span></div></div><div class="dating-points"><section><h4>Points communs</h4>${listMarkup(row.strengths,'Critères de base compatibles.')}</section><section><h4>À découvrir ensemble</h4>${listMarkup(row.explore,'Aucun contraste important détecté dans les critères utilisés.')}</section></div><button class="btn btn-primary btn-small" data-request-intro type="button">Demander une présentation</button></article>`).join('')||'<div class="dating-empty">Aucune nouvelle proposition ne correspond actuellement aux critères mutuels. Le système n’affiche jamais des profils simplement pour remplir la liste.</div>';
  recommendationsNode.querySelectorAll('[data-dating-candidate]').forEach(card=>{
    const button=card.querySelector('[data-request-intro]');
    button?.addEventListener('click',()=>requestIntroduction(card.dataset.datingCandidate,button));
  });
}

function introductionStatusLabel(row){
  if(row.status==='accepted')return 'Présentation acceptée';
  if(row.status==='declined')return 'Présentation fermée';
  if(row.status==='closed')return 'Conversation fermée';
  return row.requested_by_me?'En attente de réponse':'À vous de décider';
}
...
async function photoGate(row,node){
  ...
  const canConsent=sent>=threshold&&received>=threshold;
  ...
  node.querySelector('[data-photo-consent]')?.addEventListener('click',async event=>{
    event.currentTarget.disabled=true;
    try{const {error:rpcError}=await getSupabase().rpc('dating_request_photo_reveal',{p_introduction_id:row.id});if(rpcError)throw rpcError;await photoGate(row,node);}
    catch(err){fail(err,'Impossible d’enregistrer votre accord photo.');}
  });
  node.querySelector('[data-photo-revoke]')?.addEventListener('click',async event=>{
    event.currentTarget.disabled=true;
    try{const {error:rpcError}=await getSupabase().rpc('dating_revoke_photo_reveal',{p_introduction_id:row.id});if(rpcError)throw rpcError;await photoGate(row,node);}
    catch(err){fail(err,'Impossible de retirer votre accord photo.');}
  });
}

async function respondIntroduction(id,accept,button){
  button.disabled=true;
  try{
    const {error}=await getSupabase().rpc('dating_respond_introduction',{p_introduction_id:id,p_accept:accept});
    if(error)throw error;
    socialStatus(statusNode,accept?'Présentation acceptée. Vous pouvez commencer à discuter.':'Présentation refusée et fermée.','success');
    await loadIntroductions();
  }catch(error){fail(error,'Impossible de répondre à cette présentation.');}
  finally{button.disabled=false;}
}

async function closeIntroduction(id,button){
  if(!confirm('Fermer cette présentation? Les messages communautaires déjà échangés ne seront pas supprimés.'))return;
  button.disabled=true;
  try{
    const {error}=await getSupabase().rpc('dating_close_introduction',{p_introduction_id:id});
    if(error)throw error;
    await loadIntroductions();
  }catch(error){fail(error,'Impossible de fermer cette présentation.');}
  finally{button.disabled=false;}
}
...
async function loadIntroductions(){
  ...
  if(error)throw error;
  const rows=Array.isArray(data)?data:[];
  introductionsNode.innerHTML=rows.map(row=>`<article class="dating-intro" data-introduction="${escapeHtml(row.id)}"><div class="dating-intro-head"><div><span class="dating-badge ${row.status==='accepted'?'accepted':'requested'}">${escapeHtml(introductionStatusLabel(row))}</span><h3>${escapeHtml(row.other_pseudo||'Membre SINJIRA')}</h3></div></div>${row.status==='requested'&&!row.requested_by_me?'<div class="dating-actions"><button class="btn btn-primary btn-small" data-intro-accept type="button">Accepter la présentation</button><button class="btn btn-secondary btn-small" data-intro-decline type="button">Refuser</button></div>':''}${row.status==='requested'&&row.requested_by_me?'<p class="dating-help">La personne a reçu une notification. Aucun message n’est ouvert tant qu’elle n’a pas accepté.</p>':''}${row.status==='accepted'?`<div class="dating-actions"><a class="btn btn-primary btn-small" href="messages-reels.html?user=${encodeURIComponent(row.other_user_id)}">Écrire un message</a><button class="btn btn-secondary btn-small" data-intro-close type="button">Fermer la présentation</button></div><div data-photo-gate></div>`:''}${['declined','closed'].includes(row.status)?'<p class="dating-help">Cette présentation n’est plus active.</p>':''}</article>`).join('')||'<div class="dating-empty">Aucune présentation pour le moment.</div>';

  rows.forEach(row=>{
    const card=introductionsNode.querySelector(`[data-introduction="${CSS.escape(row.id)}"]`);
    if(!card)return;
    card.querySelector('[data-intro-accept]')?.addEventListener('click',event=>respondIntroduction(row.id,true,event.currentTarget));
    card.querySelector('[data-intro-decline]')?.addEventListener('click',event=>respondIntroduction(row.id,false,event.currentTarget));
    card.querySelector('[data-intro-close]')?.addEventListener('click',event=>closeIntroduction(row.id,event.currentTarget));
    const gate=card.querySelector('[data-photo-gate]');
    if(gate)photoGate(row,gate).catch(error=>console.error('[SINJIRA dating photo gate]',error));
  });
}
...
async function saveProfile(activeOverride=null){
  ...
  const {data,error}=await getSupabase().rpc('dating_save_profile',{p_input:payload});
  if(error)throw error;
  await loadOwnProfile();
  socialStatus(statusNode,activeOverride===false?'Profil de rencontre mis en pause.':data?.active?'Profil de rencontre actif et enregistré.':'Profil enregistré en mode pause.','success');
  await Promise.all([loadRecommendations(),loadIntroductions()]);
}
...
async function deleteProfile(){
  ...
  const {error}=await getSupabase().rpc('dating_delete_my_profile');
  if(error)throw error;
  ownProfile=null;
  form.reset();
  setField('preferred_age_min',18);setField('preferred_age_max',80);setField('communication_style','balanced');setField('life_rhythm','balanced');setField('social_energy','balanced');setField('distance_scope','same_region');
  socialStatus(statusNode,'Votre profil de rencontre a été supprimé.','success');
  await Promise.all([loadRecommendations(),loadIntroductions()]);
}
...
(async()=>{
  try{
    ...
    await Promise.all([loadRecommendations(),loadIntroductions()]);
    socialStatus(statusNode,`Rencontres prêtes · interface ${UI_VERSION}.`,'success');
  }catch(error){
    if(error?.message!=='RULES_REQUIRED'&&error?.message!=='Connexion requise')fail(error,'Rencontres SINJIRA™ n’a pas pu terminer sa vérification.');
  }
})();
