...
function reportTarget({network='real',targetType,targetId,label}){
  openSocialReport({
    network,targetType,targetId,label,statusNode:status,
...
function bind(posts,comments){
  feed.querySelectorAll('[data-post]').forEach(card=>{
    ...
    card.querySelector('[data-report]')?.addEventListener('click',()=>{
      if(post)reportTarget({targetType:'post',targetId:id,label:'cette publication'});
    });
    ...
    card.querySelectorAll('[data-comment]').forEach(commentCard=>{
      ...
      commentCard.querySelector('[data-report-comment]')?.addEventListener('click',()=>{
        if(comment)reportTarget({targetType:'comment',targetId:comment.id,label:'ce commentaire'});
      });
    });
