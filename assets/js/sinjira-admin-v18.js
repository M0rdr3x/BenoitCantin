...
function renderStoryClaims(storyId){
 ...
 box.querySelectorAll('[data-remove-story-claim]').forEach(b=>b.addEventListener('click',async()=>{if(!confirm('Retirer ce fait de provenance?'))return;try{await call('remove_story_claim',{claim_id:b.dataset.removeStoryClaim});await canonProvenance();renderStoryClaims(storyId)}catch(e){alert(e.message)}}));
}
...
function renderStorySegments(storyId){
 ...
 box.querySelectorAll('[data-remove-story-segment]').forEach(b=>b.addEventListener('click',async()=>{if(!confirm('Retirer ce segment de continuité?'))return;try{await call('remove_extended_story_segment',{segment_id:b.dataset.removeStorySegment});await extendedStories();renderStorySegments(storyId)}catch(e){alert(e.message)}}));
}
...
function bindCanonProvenance(){
 ...
 cf?.addEventListener('submit',async e=>{e.preventDefault();const storyId=cf.elements.story_id.value;if(!storyId)return alert('Sélectionnez d’abord une Chronique.');const claim={id:cf.elements.id.value||null,story_id:storyId,claim_key:cf.elements.claim_key.value,claim_type:cf.elements.claim_type.value,source_id:cf.elements.source_id.value||null,verification_status:cf.elements.verification_status.value,statement:cf.elements.statement.value,author_note:cf.elements.author_note.value};try{await call('save_story_claim',{claim});cf.reset();cf.elements.id.value='';cf.elements.story_id.value=storyId;await canonProvenance();renderStoryClaims(storyId)}catch(err){alert(err.message)}});
}
