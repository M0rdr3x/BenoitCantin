...
function renderStorySegments(storyId){const box=document.querySelector('[data-story-segment-list]'),sf=document.querySelector('[data-story-segment-form]');if(sf)sf.elements.story_id.value=storyId||'';if(!box)return;const st=extendedStoriesCache.find(x=>x.id===storyId),rows=Array.isArray(st?.sinjira_story_character_presence)?st.sinjira_story_character_presence:[];box.innerHTML=rows.map(seg=>`<article class="admin-v18-row"><strong>${escapeHtml(seg.characters?.public_name||'Personnage')} · ${escapeHtml(seg.segment_key||'segment')}</strong><p>${escapeHtml(seg.presence_kind||'scene')} · ${escapeHtml(seg.sinjira_world_locations?.name||seg.location_name||'Lieu non normalisé')} · ${escapeHtml(seg.certainty||'confirmed')}</p><small>${seg.starts_at?escapeHtml(new Date(seg.starts_at).toLocaleString('fr-CA')):'Début à compléter'} → ${seg.ends_at?escapeHtml(new Date(seg.ends_at).toLocaleString('fr-CA')):'Fin à compléter'}</small>${seg.segment_key==='primary'?'<p><em>Segment principal — géré par la fiche de Chronique.</em></p>':`<div class="admin-v18-actions"><button class="btn btn-secondary btn-small" data-edit-story-segment="${seg.id}">Modifier</button><button class="btn btn-secondary btn-small" data-remove-story-segment="${seg.id}">Retirer</button></div>`}</article>`).join('')||'<p>Aucun segment. La fiche principale créera automatiquement le segment primary pour une Chronique liée à un personnage.</p>';box.querySelectorAll('[data-edit-story-segment]').forEach(b=>b.addEventListener('click',()=>fillStorySegment(rows.find(x=>x.id===b.dataset.editStorySegment))));box.querySelectorAll('[data-remove-story-segment]').forEach(b=>b.addEventListener('click',async()=>{if(!confirm('Retirer ce segment de continuité?'))return;try{await call('remove_extended_story_segment',{segment_id:b.dataset.removeStorySegment});await extendedStories();renderStorySegments(storyId)}catch(e){alert(e.message)}}))}
