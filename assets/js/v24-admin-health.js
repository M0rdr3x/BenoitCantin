import {getSupabase,getCurrentUser,isSinjiraOwner,escapeHtml,requireUser} from './sinjira-supabase.js';

const PLATFORM_VERSION='24.4.13';
const ADMIN_SECURITY_VERSION='24.4.67';
const FRACTURE_ENGINE_VERSION='24.4.6';

// Ce module est chargé automatiquement sur /admin/sinjira/. Le garde central
// redirige vers /compte/mfa.html si le compte a activé TOTP mais reste en aal1.
await requireUser();
...
if(box){
  ...
  async function edgeState(name,body){const {data,error}=await s.functions.invoke(name,{body});if(!error&&data?.ok!==false)return{name:`Edge: ${name}`,state:'ok',detail:`Joignable${data?.version?` · ${data.version}`:''}`};if(data?.code==='MFA_REQUIRED')return{name:`Edge: ${name}`,state:'restricted',detail:'AAL2 requis par la sécurité administrateur'};if(error&&/Failed|404|not found|FunctionsFetchError/i.test(`${error?.name||''} ${error?.message||''}`))return{name:`Edge: ${name}`,state:'missing',detail:'Non déployée ou non joignable'};return{name:`Edge: ${name}`,state:'degraded',detail:'Réponse incomplète'}}
