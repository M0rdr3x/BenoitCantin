...
function portrait(src,name){if(!src)return '';return `<div class="character-account-portrait"><img src="${escapeHtml(src)}" alt="Portrait du personnage ${escapeHtml(name||'SINJIRA™')}" loading="eager"></div>`}
...
function renderOwnerFallback(hasSubmission=false){
 renderCharacter({public_name:'AbyssTime',public_description:'Personnage officiel associé au compte de Benoit Cantin.',portrait_path:'/assets/media/characters/abysstime.webp',status:'assigned',visible_to_user:true,novel_note:'SINJIRA — Livre II (titre à confirmer)'},[]);
 if(status){status.hidden=false;status.textContent=hasSubmission?'AbyssTime est affiché. Le correctif Supabase V24.3 rattache aussi votre dernier questionnaire au même personnage.':'AbyssTime est affiché. La synchronisation serveur V24.3 finalise sa fiche persistante.';status.dataset.statusType='info'}
}
...
}catch(e){if(e?.message!=='Connexion requise'&&status){status.textContent=e?.message||'Erreur.';status.hidden=false;status.dataset.statusType='error'}}})();
