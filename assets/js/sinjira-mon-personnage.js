...
function renderOwnerFallback(hasSubmission=false,loading=false){
...
(async()=>{try{
  ...
  // Le propriétaire ne doit jamais rester devant un écran vide pendant une panne ou une migration.
  // La fiche locale est affichée immédiatement, puis remplacée par la fiche persistante si Supabase répond.
  if(owner)renderOwnerFallback(false,true);

  if(owner){try{await s.rpc('ensure_sinjira_owner_character')}catch(_){/* fallback local déjà affiché */}}
  ...
  if(ch){renderCharacter(ch,events);if(status&&owner){status.hidden=false;status.textContent='AbyssTime est synchronisé avec la fiche persistante de votre Compte SINJIRA™.';status.dataset.statusType='success'}return}
  if(owner){renderOwnerFallback(Boolean(sub),false);return}
  ...
  try{
    const user=await requireUser('/compte/connexion.html');
    if(isSinjiraOwner(user)){renderOwnerFallback(false,false);return}
