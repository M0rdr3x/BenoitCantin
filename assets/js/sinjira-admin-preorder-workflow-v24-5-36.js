...
function printLogisticsSheet() {
  ...
  const localPrintButton = printWindow.document.getElementById('sinjira-local-print');
  localPrintButton?.addEventListener('click', () => printWindow.print());
  try { printWindow.opener = null; } catch {}
  printWindow.focus();
  setLogisticsStatus(`Feuille locale durcie préparée pour ${rows.length} réservation(s). Aucun script inline, fichier ou appel réseau n’a été envoyé vers un service externe.`, 'success');
}
function bind() {
