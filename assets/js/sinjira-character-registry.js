...
async function submitToSinjira(user,{updateExisting=false}={}){const photo_path=await uploadPhoto(user);const answers=serialize(form);const {data,error}=await getSupabase().functions.invoke('submit-character-questionnaire',{body:{answers,photo_path,manual_only:true,update_existing:updateExisting}});if(error||!data?.ok)throw new Error(data?.error||error?.message||'Le dossier n’a pas pu être enregistré dans SINJIRA™.');return data}
...
async function init(){
 ...
 form.addEventListener('submit',async e=>{
   ...
   try{
     ...
     setState(result.updated?'Questionnaire d’AbyssTime mis à jour dans SINJIRA™. Le personnage existant est conservé.':result.notification_sent?'Participation enregistrée dans SINJIRA™ et avis courriel envoyé à Benoit Cantin.':'Participation enregistrée dans SINJIRA™. Elle est visible dans l’administration; aucun courriel externe n’est nécessaire pour conserver le dossier.','success');
     form.querySelectorAll('input,textarea,select,button').forEach(x=>x.disabled=true);
