...
async function dashboard(){
  ...
  const [rs,rp,rl,rr,ownerResult,adminResult]=await Promise.all([
  ...
  const isOwner=ownerResolved&&ownerResult.data===true,isAdmin=adminResolved&&adminResult.data===true;
  ...
  set('[data-account-role]',!ownerResolved||!adminResolved?'Rôle du compte non confirmé':isOwner?'Créateur SINJIRA™':isAdmin?'Administrateur SINJIRA™':'Membre SINJIRA™');
  const projectCount=projects.length,projectPlural=projectCount===1?'projet':'projets';
  set('[data-project-access-summary]',!ownerResolved||!adminResolved
    ? `${projectCount} ${projectPlural} actuellement visible${projectCount===1?'':'s'}; rôle du compte non confirmé.`
    : isOwner
      ? `${projectCount} création${projectCount===1?'':'s'} visible${projectCount===1?'':'s'} dans votre catalogue propriétaire.`
      : isAdmin
        ? `${projectCount} ${projectPlural} accessible${projectCount===1?'':'s'} avec votre rôle d’administration.`
        : `${projectCount} ${projectPlural} accessible${projectCount===1?'':'s'} avec votre compte.`);
