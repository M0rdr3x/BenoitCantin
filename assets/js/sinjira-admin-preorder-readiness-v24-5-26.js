...
function item(label, value, good = value) {
  return `<div><span>${escapeHtml(label)}</span><strong>${escapeHtml(yesNo(value))}</strong><small>${good ? 'conforme' : 'à compléter'}</small></div>`;
}
...
function renderReadiness(data) {
  ...
  const flagsLocked = data?.sales_enabled === false
    && data?.checkout_enabled === false
    && data?.payment_enabled === false
    && data?.external_fulfillment_enabled === false
    && data?.auto_conversion_allowed === false
    && data?.external_carrier_api_enabled === false
    && data?.external_shipping_purchase_enabled === false;
  ...
  card.innerHTML = `
    <div class="preorder-admin-card-head">
      <div>
        ...
        <p>Cette checklist vérifie uniquement si les informations sont assez complètes pour envisager plus tard une ouverture manuelle. <strong>Elle n’ouvre aucune vente, aucun checkout et aucun paiement. Vente toujours désactivée.</strong></p>
      </div>
