...
SELF_TEST = 'python scripts/validate_live_social_foundation_workflow_security.py --self-test'
SELF_CHECK = 'python scripts/validate_live_social_foundation_workflow_security.py'
...
TRIGGER_PATHS = (
    ...
    "'supabase/tests/live_social_realtime_eligibility_v25.test.sql'",
...
def validate_text(text: str) -> None:
    ...
    first_test = text.index(PGTAP_CHECKS[0])
    second_test = text.index(PGTAP_CHECKS[1])
    stop_index = text.index('run: supabase stop --no-backup || true')
    if not static_index < start_index < first_test < second_test < stop_index:
...
def mutations(text: str):
    ...
    yield 'chemin contrat retiré', text.replace("      - 'scripts/validate_live_social_foundation_workflow_security.py'\n", '', 2)
    ...
    yield 'validateur fondation retiré', text.replace(STATIC_CHECKS[0], 'echo fondation-retire', 1)
    yield 'validateur ledger retiré', text.replace(STATIC_CHECKS[3], 'echo ledger-retire', 1)
...
def self_test(text: str) -> None:
    ...
    for name, mutated in mutations(text):
        ...
        try:
            ...
            raise SystemExit(f'ECHEC auto-test CI fondation En direct: mutation non détectée: {name}')
    print(f'OK: {detected}/{total} mutations critiques détectées')


def main() -> int:
    if not WORKFLOW.is_file():
        print('ECHEC CI fondation En direct: workflow absent', file=sys.stderr)
        return 1
