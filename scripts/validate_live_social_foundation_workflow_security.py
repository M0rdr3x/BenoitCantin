...
def extra_mutations(text: str):
    ...
    yield 'déclencheur test fondation retiré', text.replace("      - 'supabase/tests/live_social_foundation_v25.test.sql'\n", '', 2)
    yield 'déclencheur test Realtime retiré', text.replace("      - 'supabase/tests/live_social_realtime_eligibility_v25.test.sql'\n", '', 2)
    yield 'déclencheur communauté retiré', text.replace("      - 'compte/communaute.html'\n", '', 2)
    yield 'déclencheur contrat retiré', text.replace("      - 'scripts/validate_live_social_foundation_workflow_security.py'\n", '', 2)
...
def self_test(text: str) -> None:
    common_self_test(text, validate_text, extra_mutations(text), label='fondation En direct')
