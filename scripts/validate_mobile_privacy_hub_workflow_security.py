...
SELF_TEST = 'python3 scripts/validate_mobile_privacy_hub_workflow_security.py --self-test'
SELF_CHECK = 'python3 scripts/validate_mobile_privacy_hub_workflow_security.py'
VALIDATORS = (
...
GUARD_PATH = "      - 'scripts/validate_mobile_privacy_hub_workflow_security.py'"
PRIMARY_PATH = "      - 'scripts/validate_mobile_native_privacy_hub_v25.py'"
PUSH_PATHS = (
    "      - 'mobile-native/**'",
    PRIMARY_PATH,
...
def validate_text(text: str) -> None:
    ...
    require(text.count(PRIMARY_PATH) == 2, 'Le validateur Vie privée doit déclencher exactement les frontières PR et push.')
...
def mutation_cases(text: str) -> tuple[tuple[str, str], ...]:
    ...
    cases = [
        ...
        ('validateur Vie privée PR retiré', text.replace(PRIMARY_PATH + '\n', '', 1)),
    ...
    for index, command in enumerate(VALIDATORS):
        cases.append((f'preuve vie privée {index + 1} retirée', text.replace(f'        run: {command}\n', '', 1)))
    return tuple(cases)
...
def main() -> int:
    ...
    validate_text(text)
    validate_repo_state()
    print('OK CI Vie privée native: runner/Python/Node/actions immuables, Git read-only, scripts npm désactivés, aucun faux cache et aucune opération Supabase distante.')
    return 0
