...
def validate_text(text: str) -> None:
    required = [
        ...
        'python scripts/validate_sensitive_aal2_workflow_security.py --self-test',
        'python scripts/validate_sensitive_aal2_workflow_security.py',
        'python scripts/validate_sensitive_aal2_smoke.py',
        'supabase start',
        'python3 scripts/smoke_sensitive_aal2_local.py',
...
def self_test(text: str) -> None:
    mutations = {
        ...
        'déclencheur contrat push retiré': text.replace(CONTRACT_TRIGGER + '\n', '', 1),
        'auto-test retiré': text.replace(
            '      - name: Auto-tester le contrat CI AAL2 sensible\n'
            '        run: python scripts/validate_sensitive_aal2_workflow_security.py --self-test\n\n',
            '',
            1,
        ),
        'contrat smoke retiré': text.replace('        run: python scripts/validate_sensitive_aal2_smoke.py\n', '', 1),
        'Supabase local retiré': text.replace('        run: supabase start\n', '', 1),
        'smoke AAL2 retiré': text.replace('            python3 scripts/smoke_sensitive_aal2_local.py\n', '', 1),
