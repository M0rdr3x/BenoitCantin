...
CHALLENGE_MIGRATION_TRIGGER = "      - 'supabase/migrations/**challenge**.sql'"
SESSION_BINDING_MIGRATION_TRIGGER = (
    "      - 'supabase/migrations/20260910193000_sinjira_v25_device_challenge_session_binding.sql'"
)
CHALLENGE_TRIGGER = "      - 'supabase/tests/device_challenge_continuity_v25.test.sql'"
SESSION_BINDING_TRIGGER = "      - 'supabase/tests/device_challenge_session_binding_v25.test.sql'"
SECURITY_CONTEXT_TRIGGER = "      - 'supabase/functions/security-context/**'"
...
SESSION_PGTAP_RUN = (
    '        run: supabase test db supabase/tests/device_challenge_session_binding_v25.test.sql --local\n'
)
...
def count_exact(text: str, line: str) -> int:
    return sum(1 for candidate in text.splitlines() if candidate.strip() == line.strip())
...
def validate_text(text: str) -> None:
    required = [
        ...
        'supabase test db supabase/tests/device_challenge_session_binding_v25.test.sql --local',
    ...
    for trigger, label in (
        ...
        (REBIND_TRIGGER, 'contrat de réassociation'),
        (SECURITY_MIGRATION_TRIGGER, 'migrations sécurité'),
        (CHALLENGE_MIGRATION_TRIGGER, 'migrations challenge'),
        (SESSION_BINDING_MIGRATION_TRIGGER, 'migration challenge/session exacte'),
        (CHALLENGE_TRIGGER, 'pgTAP challenge'),
        (SESSION_BINDING_TRIGGER, 'pgTAP liaison session'),
        (SECURITY_CONTEXT_TRIGGER, 'Edge security-context'),
    ):
        if text.count(trigger) != 2:
            fail(f'{label} doit déclencher le workflow exactement sur PR et push')

    if count_exact(text, 'run: supabase start') != 1:
        fail('supabase start doit être exécuté exactement une fois')
    if count_exact(text, 'run: supabase stop --no-backup || true') != 1:
        fail('supabase stop local doit être exécuté exactement une fois')
    if count_exact(text, SESSION_PGTAP_RUN.strip()) != 1:
        fail('le pgTAP liaison challenge/session doit être exécuté exactement une fois')
    ...
    if re.search(r'(^|\s)set\s+-[^\n]*x', text):
        fail('les traces shell x sont interdites')
    ...
    if len(uses_targets) != 3 or set(uses_targets) != expected_actions:
        fail(f'actions réutilisables inattendues: {uses_targets}')
    ...
    session_pg = text.index('supabase test db supabase/tests/device_challenge_session_binding_v25.test.sql --local')
    smoke = text.index('python3 scripts/smoke_device_challenge_continuity_local.py')
    if not (rebind_contract < base_pg < challenge_pg < session_pg < smoke):
        fail('ordre attendu: contrat rebind, pgTAP Coffre, pgTAP challenge, pgTAP session, puis smoke HTTP')


def self_test(text: str) -> None:
    # Un self-test mutationnel n'a de valeur que si le baseline est valide.
    validate_text(text)
    ...
    mutations = {
        ...
        'continue-on-error': text.replace('timeout-minutes: 20', 'timeout-minutes: 20\n    continue-on-error: true', 1),
        'traces shell': text.replace('          set -euo pipefail', '          set -euxo pipefail', 1),
        'réseau curl': text.replace('          status_env=', '          curl https://example.invalid\n          status_env=', 1),
        'git push': text.replace('          status_env=', '          git push origin main\n          status_env=', 1),
        ...
        'déclencheur migrations challenge retiré': text.replace(CHALLENGE_MIGRATION_TRIGGER + '\n', '', 1),
        'déclencheur migration session retiré': text.replace(SESSION_BINDING_MIGRATION_TRIGGER + '\n', '', 1),
        'déclencheur challenge retiré': text.replace(CHALLENGE_TRIGGER + '\n', '', 1),
        'déclencheur pgTAP session retiré': text.replace(SESSION_BINDING_TRIGGER + '\n', '', 1),
        'déclencheur security-context retiré': text.replace(SECURITY_CONTEXT_TRIGGER + '\n', '', 1),
        ...
        'pgTAP session retiré': text.replace(SESSION_PGTAP_RUN, '', 1),
    ...
    print(f'OK: baseline valide et {len(mutations)}/{len(mutations)} mutations critiques détectées')
...
def main() -> None:
    ...
    if args.self_test:
        ...
        validate_text(text)
        print('OK: contrat CI continuité challenge appareils V25 session-aware respecté')
