...
SELF_TEST = 'python3 scripts/validate_mobile_account_route_classification_workflow_security.py --self-test'
SELF_CHECK = 'python3 scripts/validate_mobile_account_route_classification_workflow_security.py'
HISTORICAL = (
...
TRIGGER_PATHS = (
    ...
    "      - 'mobile-native/NATIVE_SECONDARY_ROUTE_ALIASES_V25.md'",
    ...
    "      - 'scripts/validate_mobile_native_secondary_route_aliases_v25.py'",
    ...
    "      - 'scripts/validate_mobile_account_route_classification_workflow_security.py'",
...
def validate_text(text: str) -> None:
    ...
    forbidden = (
        ...
        'curl ', 'wget ', 'gh api', 'git push', 'continue-on-error: true', 'set -x',
    )
...
def mutation_cases(text: str) -> tuple[tuple[str, str], ...]:
    ...
    return (
        ...
        ('chemin compte retiré', text.replace("      - 'compte/*.html'\n", '', 1)),
        ('chemin App retiré', text.replace("      - 'mobile-native/App.tsx'\n", '', 1)),
        ('chemin lockfile retiré', text.replace("      - 'mobile-native/package-lock.json'\n", '', 1)),
        ('chemin garde retiré', text.replace("      - 'scripts/validate_mobile_account_route_classification_workflow_security.py'\n", '', 1)),
        ('auto-test retiré', text.replace(f'        run: {SELF_TEST}\n', '', 1)),
        ('contrat retiré', text.replace(f'        run: {SELF_CHECK}\n', '', 1)),
        ('classification retirée', text.replace(f'        run: {HISTORICAL[0]}\n', '', 1)),
        ('routeur retiré', text.replace(f'        run: {HISTORICAL[1]}\n', '', 1)),
        ('alias retirés', text.replace(f'        run: {HISTORICAL[2]}\n', '', 1)),
        ('accueil retiré', text.replace(f'        run: {HISTORICAL[3]}\n', '', 1)),
        ('sécurité retirée', text.replace(f'        run: {HISTORICAL[4]}\n', '', 1)),
        ('navigation retirée', text.replace(f'        run: {HISTORICAL[5]}\n', '', 1)),
        ('partage retiré', text.replace(f'        run: {HISTORICAL[6]}\n', '', 1)),
        ('challenge retiré', text.replace(f'        run: {HISTORICAL[7]}\n', '', 1)),
        ('garde secrets retiré', text.replace(f'        run: {HISTORICAL[8]}\n', '', 1)),
        ('scripts npm réactivés', text.replace(HISTORICAL[9], 'npm install --no-audit --no-fund', 1)),
        ('vault retiré', text.replace(f'        run: {HISTORICAL[10]}\n', '', 1)),
        ('typecheck retiré', text.replace(f'        run: {HISTORICAL[11]}\n', '', 1)),
...
def main() -> int:
    ...
    validate_repo_state()
    print(
        'OK CI classification routes compte: runner/Python/Node/actions immuables, Git read-only, '
