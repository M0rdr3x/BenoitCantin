...
def validate(contents:dict[str,str])->None:
    ...
    for boundary_marker in (
        ...
        "elsesinjira_v25_internal.has_sinjira_product(p_product_slug,p_user_id)",
        "public.sinjira_my_age_band()in('adult','youth')",
    ...
    for project_marker in (
        ...
        "executeformat('droppolicyifexists%ionpublic.documents',policy_row.policyname)",
        ...
        "orsinjira_catalog_internal.project_access_rank(id,(selectauth.uid()))>=20",
    ...
    for marker in (
        ...
        "jsonb_build_object('role','service_role')::text,true);insertintopublic.orders(id,user_id,order_number,status,currency,total_cents)values('fd100000-0000-4000-8000-000000000013','f3000000-0000-4000-8000-000000000003','test-child-paid-product-001'",
        ...
        "lewrapperproduitnavigateurmasqueunecommandepaidà11–12",
        ...
        "lewrapperproduitnavigateurmasqueunentitlementà11–12",
...
def main()->None:
    ...
    if args.self_test:
        mutations={
            ...
            "balayage policies documents sans suppression":("project_access_migration","execute format(\n      'drop policy if exists %I on public.documents',\n      policy_row.policyname\n    );","perform policy_row.policyname;"),
            ...
            "fixture achat enfant sans contexte serveur":("test","jsonb_build_object('role','service_role')::text,\n  true\n);\n\ninsert into public.orders(id,user_id,order_number,status,currency,total_cents)\nvalues(\n  'fd100000-0000-4000-8000-000000000013',","jsonb_build_object('role','authenticated')::text,\n  true\n);\n\ninsert into public.orders(id,user_id,order_number,status,currency,total_cents)\nvalues(\n  'fd100000-0000-4000-8000-000000000013',"),
            "fixture entitlement enfant sans contexte serveur":("test","jsonb_build_object('role','service_role','fixture','child_entitlement')::text","jsonb_build_object('role','authenticated','fixture','child_entitlement')::text"),
            ...
            "accès player draft retiré de la policy":("project_access_migration","or sinjira_catalog_internal.project_access_rank(id,(select auth.uid()))>=20",""),
            "accès player draft retiré du catalogue":("project_access_migration","or sinjira_catalog_internal.project_access_rank(p.id,uid)>=20",""),
