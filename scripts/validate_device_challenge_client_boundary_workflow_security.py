#!/usr/bin/env python3
"""Security contract for the V25 multi-device client-boundary workflow."""
...
WORKFLOW = Path('.github/workflows/sinjira-device-challenge-client-boundary-v25.yml')
...
CONTRACT_STEP = (
    '      - name: Vérifier le contrat CI frontière client appareils\n'
    '        run: python scripts/validate_device_challenge_client_boundary_workflow_security.py\n'
)
...
def self_test(text: str) -> None:
    mutations = {
        ...
        'démarrage Supabase': text.replace(
            '      - name: Verrouiller la frontière client multi-appareils\n',
            '      - name: Mauvais démarrage Supabase\n        run: supabase start\n\n      - name: Verrouiller la frontière client multi-appareils\n',
            1,
        ),
...
def main() -> None:
    ...
    if args.self_test:
        ...
        validate_text(text)
        print('OK: contrat CI frontière client multi-appareils V25 respecté')
