...
SELF_TEST = 'python3 scripts/validate_mobile_secondary_route_aliases_workflow_security.py --self-test'
SELF_CHECK = 'python3 scripts/validate_mobile_secondary_route_aliases_workflow_security.py'
VALIDATORS = (
...
GUARD_PATH = "      - 'scripts/validate_mobile_secondary_route_aliases_workflow_security.py'"
PUSH_PATHS = (
    "      - 'mobile-native/**'",
    "      - 'compte/blocages.html'",
    "      - 'compte/regles-communaute.html'",
    "      - 'compte/moderation.html'",
    "      - 'assets/js/sinjira-moderation-appeals-v24-4-90.js'",
    "      - 'scripts/validate_mobile_native_*.py'",
    "      - 'scripts/validate_moderation_appeals_v24_4_90.py'",
...
def mutation_cases(text: str) -> tuple[tuple[str, str], ...]:
    ...
    for index, command in enumerate(VALIDATORS):
        cases.append((f'preuve alias {index + 1} retirée', text.replace(f'        run: {command}\n', '', 1)))
    return tuple(cases)
...
def main() -> int:
    ...
    validate_text(text)
    validate_repo_state()
    print('OK CI alias secondaires natifs: runner/Python/Node/actions immuables, Git read-only, scripts npm désactivés, aucun faux cache et aucune opération Supabase distante.')
    return 0
