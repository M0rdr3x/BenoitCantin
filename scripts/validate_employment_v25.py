...
def main()->int:
    ...
    for operation in ('isolation SELECT profil A depuis B','isolation UPDATE profil A depuis B','isolation DELETE profil A depuis B','isolation SELECT candidature A depuis B','isolation UPDATE candidature A depuis B','isolation DELETE candidature A depuis B'):
        require(operation in smoke,f'preuve cross-user manquante: {operation}')
    require('B a pu insérer un profil pour A' in smoke,'test INSERT cross-user profil absent')
    require('B a pu créer une candidature pour A' in smoke,'test INSERT cross-user candidature absent')

    print('OK emploi V25: production classifiée, RLS propriétaire, zéro fausse offre/persistance navigateur et smoke local Auth→PostgREST→RLS cross-user verrouillé.')
    return 0
