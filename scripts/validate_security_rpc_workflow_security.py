...
WORKFLOW = ROOT / '.github/workflows/sinjira-security-rpc-v24-5-10.yml'

CHECKOUT_SHA = 'd23441a48e516b6c34aea4fa41551a30e30af803'
SETUP_PYTHON_SHA = 'ece7cb06caefa5fff74198d8649806c4678c61a1'
PYTHON_VERSION = '3.12.14'
SELF_TEST = 'python scripts/validate_security_rpc_workflow_security.py --self-test'
META = 'python scripts/validate_security_rpc_workflow_security.py'
...
TRIGGER = "      - 'scripts/validate_security_rpc_workflow_security.py'\n"
...
def validate_text(text: str) -> list[str]:
    ...
    required = (
        SELF_TEST, META, SECURITY, GLOBAL_ADMIN, PREORDER_ADMIN, PREORDER_HARDENING,
        SITE, SUPABASE, FREE_ONLY, LEDGER, MANIFEST,
    )
    ...
    need(text.count(f'        run: {LEDGER}\n') == 1, 'ledger production doit rester une étape directe, unique et bloquante')
    need(text.count(f'        run: {MANIFEST}\n') == 1, 'manifeste production doit rester une étape directe, unique et bloquante')
    need(text.count(f'        run: {FREE_ONLY}\n') == 1, 'preuve mode gratuit doit rester une étape directe et unique')

    positions = [text.find(command) for command in (
        SECURITY, GLOBAL_ADMIN, PREORDER_ADMIN, PREORDER_HARDENING,
        SITE, SUPABASE, FREE_ONLY, LEDGER, MANIFEST,
    )]
    need(all(position >= 0 for position in positions), 'preuves locales et production doivent toutes être présentes')
    need(positions == sorted(positions), 'ordre requis: contrats sécurité -> site -> Supabase -> mode gratuit -> ledger -> manifeste')
...
def self_test(text: str) -> None:
    ...
    mutations = {
        ...
        'contrat sécurité retiré': text.replace(f'        run: {SECURITY}\n', '', 1),
    ...
    if source_errors:
        raise SystemExit('ERREUR auto-test workflow sécurité RPC: workflow source invalide: ' + '; '.join(source_errors))

    for label, mutated in mutations.items():
        if mutated == text:
            raise SystemExit(f'ERREUR auto-test workflow sécurité RPC: mutation sans effet: {label}')
        if not validate_text(mutated):
            raise SystemExit(f'ERREUR auto-test workflow sécurité RPC: mutation non détectée: {label}')

    print(f'OK auto-test workflow sécurité RPC: {len(mutations)}/{len(mutations)} affaiblissements critiques détectés.')


def main() -> int:
    parser = argparse.ArgumentParser(description='Valide le durcissement du workflow RPC sécurité.')
    parser.add_argument('--self-test', action='store_true')
    args = parser.parse_args()

    if not WORKFLOW.is_file():
        print(f'ÉCHEC workflow sécurité RPC: fichier absent: {WORKFLOW.relative_to(ROOT)}')
        return 1
    ...
    if errors:
        print(f'ÉCHEC workflow sécurité RPC: {len(errors)} problème(s).')
        for error in errors:
            print('- ' + error)
        return 1

    print(
        'OK workflow sécurité RPC: actions/runtimes immuables, credentials non persistés, '
        'preuves locales dont mode gratuit avant ledger/manifeste production bloquants.'
    )
    return 0
