...
def main():
    ...
    if len(TARGETS)!=5 or len(set(TARGETS))!=5: errors.append('La liste V24.5.14 doit contenir exactement 5 RPC uniques.')
    if 'moderation_content_visible' in low: errors.append('Le helper RLS moderation_content_visible ne doit pas être déplacé par V24.5.14.')
    if re.search(r'grant\s+execute\s+on\s+function\s+(?:public|sinjira_user_rights_internal)\..*?\s+to\s+anon',low,re.S): errors.append('V24.5.14 ne doit jamais accorder EXECUTE à anon.')
    ...
    if len(rows)<145: errors.append(f'Ledger historique incomplet: {len(rows)} migrations, au moins 145 attendues.')
    if rows.count(row)!=1: errors.append('Le ledger doit contenir exactement une occurrence de V24.5.14.')
    if row in rows and rows.index(row)!=144: errors.append('V24.5.14 doit conserver sa position historique 145.')

    for marker in ['5 rpc','5/5','security invoker','sinjira_user_rights_internal','145 migrations','auth.uid','service_role','révision humaine','l’humain avant tout','aucune nouvelle donnée personnelle']:
        if marker not in doc: errors.append(f'Document V24.5.14 incomplet: {marker}')
    for token in ['stripe','paypal','canadapost','canada post','fedex','purolator','shippo','easypost','twilio','api.resend.com']:
    ...
    if errors:
        ...
        for e in errors: print('- '+e)
        return 1
    print('OK V24.5.14 historique: 5 RPC Confidentialité/Appels isolées, wrappers SECURITY INVOKER, anon révoqué et droits humains conservés.')
    return 0
