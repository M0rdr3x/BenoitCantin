...
def main():
    errors=[]
    for p in [MIG1,MIG2,LEDGER,DOC,ADMIN,PUBLIC,ADMIN_LOADER,PUBLIC_LOADER,SCHEMA,POLICY,TEST]:
        if not p.exists(): errors.append(f'Fichier absent: {p.relative_to(ROOT)}')
    if errors:
        for e in errors: print('- '+e)
        return 1

    m1=read(MIG1); m2=read(MIG2); sql=flat(m1+'\n'+m2); doc=read(DOC).lower(); admin=read(ADMIN).lower(); public=read(PUBLIC).lower()
    ...
    if len(rows)!=159: errors.append(f'Ledger: {len(rows)} migrations au lieu de 159.')
    for row in expected:
        if rows.count(row)!=1: errors.append(f'Ledger V24.5.27 absent/dupliqué: {row}')
    if not rows or rows[-1]!=expected[-1]: errors.append('La migration de précision V24.5.27 doit être la dernière version courante.')

    for marker in ['159 migrations','aucun profil fiscal','mfa/aal2','14,975 %','external_tax_api_enabled = false','billing_authoritative = false','final_tax_confirmation_required = true','aucun paiement','livraison à la charge du client','0 $ de frais de livraison']:
    ...
    for marker in ['select plan(','preorder_tax_estimate_profiles','numeric_scale','security invoker','require_sinjira_admin_aal2','external_tax_api_enabled','billing_authoritative','select * from finish()','rollback;']:
    ...
    if errors:
        ...
        for e in errors: print('- '+e)
        return 1
    print('OK V24.5.27: estimation fiscale indicative, taux décimaux précis, publication humaine MFA, aucun taux par défaut, aucune API externe, aucune facturation et ledger 159 synchronisé.')
    return 0
