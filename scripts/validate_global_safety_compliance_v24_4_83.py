...
DOCS=[
 ROOT/'docs/compliance/INTERNATIONAL_COMPLIANCE_MATRIX.md', ROOT/'docs/compliance/EFVP.md',
 ROOT/'docs/compliance/CHILD_SAFETY_RISK_ASSESSMENT.md', ROOT/'docs/compliance/ILLEGAL_CONTENT_RISK_ASSESSMENT.md',
 ROOT/'docs/compliance/DATA_RETENTION_SCHEDULE.md', ROOT/'docs/compliance/PRIVACY_INCIDENT_RESPONSE.md',
...
privacy=read(PRIVACY); governance=read(GOVERNANCE); account_privacy=read(ACCOUNT_PRIVACY); privacy_center=read(PRIVACY_CENTER); privacy_js=read(PRIVACY_JS); legal=read(LEGAL)
docs='\n'.join(read(p) for p in DOCS); compact=''.join(mig.lower().split())
alltext='\n'.join((mig,test,signup_js,signup_html,privacy,governance,account_privacy,privacy_center,privacy_js,legal,docs)).lower()
...
print('OK V24.4.83: 13+ + RPRP/gouvernance + droits vie privée + incidents 5 ans + escalade sécurité + gouvernance internationale, sans service payant.')
