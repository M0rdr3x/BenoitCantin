...
SELF_TEST = 'python3 scripts/validate_mobile_alerts_hub_workflow_security.py --self-test'
SELF_CHECK = 'python3 scripts/validate_mobile_alerts_hub_workflow_security.py'
HISTORICAL = (
...
TRIGGER_PATHS = (
    ...
    "      - 'mobile-native/NATIVE_ALERTS_HUB_V25.md'",
    ...
    "      - 'scripts/validate_mobile_alerts_hub_workflow_security.py'",
...
def mutation_cases(text: str) -> tuple[tuple[str, str], ...]:
    ...
    cases = [
        ...
        ('chemin Alertes retiré', text.replace("      - 'mobile-native/NativeAlertsHub.tsx'\n", '', 1)),
        ('chemin lockfile retiré', text.replace("      - 'mobile-native/package-lock.json'\n", '', 1)),
        ('chemin garde retiré', text.replace("      - 'scripts/validate_mobile_alerts_hub_workflow_security.py'\n", '', 1)),
        ('auto-test retiré', text.replace(f'        run: {SELF_TEST}\n', '', 1)),
        ('contrat retiré', text.replace(f'        run: {SELF_CHECK}\n', '', 1)),
        ('scripts npm réactivés', text.replace(HISTORICAL[10], 'npm install --no-audit --no-fund', 1)),
    ...
    for index, command in enumerate(HISTORICAL[:10]):
        cases.append((f'preuve historique {index + 1} retirée', text.replace(f'        run: {command}\n', '', 1)))
    cases.extend([
        ('vault retiré', text.replace(f'        run: {HISTORICAL[11]}\n', '', 1)),
        ('typecheck retiré', text.replace(f'        run: {HISTORICAL[12]}\n', '', 1)),
    ])
    return tuple(cases)
...
def main() -> int:
    ...
    validate_text(text)
    validate_repo_state()
    print('OK CI Hub Alertes natif: runner/Python/Node/actions immuables, Git read-only, scripts npm désactivés, aucun faux cache et aucune opération Supabase distante.')
    return 0
