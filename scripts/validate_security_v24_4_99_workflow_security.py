...
WORKFLOW = ROOT / '.github/workflows/sinjira-security-v24-4-99.yml'
...
def validate_text(text: str) -> list[str]:
    ...
    require(errors, text.count('persist-credentials: false') == 1, 'checkout doit désactiver la persistance des credentials')
    ...
    required_runs = (
        'python scripts/validate_security_v24_4_99.py',
        'python scripts/validate_auth.py',
        'python scripts/validate_production_migration_ledger.py',
        'python scripts/validate_security_v24_4_99_workflow_security.py --self-test',
        'python scripts/validate_security_v24_4_99_workflow_security.py\n',
    )
...
def run_self_tests(text: str) -> None:
    cases = {
        ...
        'ledger retiré': text.replace('        run: python scripts/validate_production_migration_ledger.py\n', '', 1),
        'auth retiré': text.replace('        run: python scripts/validate_auth.py\n', '', 1),
    }
    for name, mutated in cases.items():
        if not validate_text(mutated):
            raise SystemExit(f'ERREUR auto-test V24.4.99: mutation non détectée: {name}')
    print(f'OK auto-tests V24.4.99: {len(cases)} affaiblissements critiques détectés.')
...
def main() -> int:
    ...
    if not WORKFLOW.is_file():
        raise SystemExit(f'ERREUR V24.4.99: workflow absent: {WORKFLOW.relative_to(ROOT)}')
    ...
    if errors:
        for error in errors:
            print(f'ERREUR sécurité V24.4.99: {error}')
        return 1
    print('OK sécurité V24.4.99: actions immuables, runtime figé, credentials non persistés et contrôles Auth/ledger conservés.')
    return 0
