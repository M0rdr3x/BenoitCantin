...
WORKFLOW = ROOT / '.github/workflows/sinjira-family-playtest-rpc-v24-5-17.yml'

CHECKOUT_SHA = 'd23441a48e516b6c34aea4fa41551a30e30af803'
SETUP_PYTHON_SHA = 'ece7cb06caefa5fff74198d8649806c4678c61a1'
PYTHON_VERSION = '3.12.14'
SELF_TEST = 'python scripts/validate_family_playtest_rpc_workflow_security.py --self-test'
META = 'python scripts/validate_family_playtest_rpc_workflow_security.py'
FAMILY = 'python scripts/validate_family_playtest_rpc_v24_5_17.py'
...
TRIGGER = "      - 'scripts/validate_family_playtest_rpc_workflow_security.py'\n"
...
def validate_text(text: str) -> list[str]:
    ...
    required = (SELF_TEST, META, FAMILY, PARALLEL, SITE, SUPABASE, FREE_ONLY, LEDGER, MANIFEST)
    ...
    positions = [text.find(command) for command in (
        FAMILY, PARALLEL, SITE, SUPABASE, FREE_ONLY, LEDGER, MANIFEST,
    )]
    need(all(position >= 0 for position in positions), 'preuves locales et production doivent toutes être présentes')
    need(
        positions == sorted(positions),
        'ordre requis: famille/playtest -> Monde parallèle -> site -> Supabase -> mode gratuit -> ledger -> manifeste',
    )
...
def self_test(text: str) -> None:
    ...
    mutations = {
        ...
        'contrat famille/playtest retiré': text.replace(f'        run: {FAMILY}\n', '', 1),
        'historique Monde parallèle retiré': text.replace(f'        run: {PARALLEL}\n', '', 1),
    ...
    if source_errors:
        raise SystemExit('ERREUR auto-test workflow famille/playtest: workflow source invalide: ' + '; '.join(source_errors))

    for label, mutated in mutations.items():
        if mutated == text:
            raise SystemExit(f'ERREUR auto-test workflow famille/playtest: mutation sans effet: {label}')
        if not validate_text(mutated):
            raise SystemExit(f'ERREUR auto-test workflow famille/playtest: mutation non détectée: {label}')

    print(f'OK auto-test workflow famille/playtest: {len(mutations)}/{len(mutations)} affaiblissements critiques détectés.')


def main() -> int:
    parser = argparse.ArgumentParser(description='Valide le durcissement du workflow RPC famille/playtest.')
    parser.add_argument('--self-test', action='store_true')
    args = parser.parse_args()

    if not WORKFLOW.is_file():
        print(f'ÉCHEC workflow famille/playtest: fichier absent: {WORKFLOW.relative_to(ROOT)}')
        return 1
    ...
    if errors:
        print(f'ÉCHEC workflow famille/playtest: {len(errors)} problème(s).')
        for error in errors:
            print('- ' + error)
        return 1

    print(
        'OK workflow famille/playtest: actions/runtimes immuables, credentials non persistés, '
        'preuves famille/playtest et mode gratuit avant ledger/manifeste production bloquants.'
    )
    return 0
