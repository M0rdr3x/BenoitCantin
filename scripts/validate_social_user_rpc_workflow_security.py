...
WORKFLOW = ROOT / '.github/workflows/sinjira-social-user-rpc-v24-5-15.yml'

CHECKOUT_SHA = 'd23441a48e516b6c34aea4fa41551a30e30af803'
SETUP_PYTHON_SHA = 'ece7cb06caefa5fff74198d8649806c4678c61a1'
PYTHON_VERSION = '3.12.14'
SELF_TEST = 'python scripts/validate_social_user_rpc_workflow_security.py --self-test'
META = 'python scripts/validate_social_user_rpc_workflow_security.py'
...
TRIGGER = "      - 'scripts/validate_social_user_rpc_workflow_security.py'\n"
...
def validate_text(text: str) -> list[str]:
    ...
    required = (
        SELF_TEST, META, SOCIAL, RIGHTS, LIFE, DATING, SITE, SUPABASE, FREE_ONLY, LEDGER, MANIFEST,
    )
    ...
    positions = [text.find(command) for command in (
        SOCIAL, RIGHTS, LIFE, DATING, SITE, SUPABASE, FREE_ONLY, LEDGER, MANIFEST,
    )]
    need(all(position >= 0 for position in positions), 'preuves locales et production doivent toutes être présentes')
    need(
        positions == sorted(positions),
        'ordre requis: social -> droits -> Histoire de vie -> Rencontres -> site -> Supabase -> mode gratuit -> ledger -> manifeste',
    )
...
def self_test(text: str) -> None:
    ...
    mutations = {
        ...
        'contrat social retiré': text.replace(f'        run: {SOCIAL}\n', '', 1),
        'historique droits retiré': text.replace(f'        run: {RIGHTS}\n', '', 1),
    ...
    if source_errors:
        raise SystemExit('ERREUR auto-test workflow social utilisateur: workflow source invalide: ' + '; '.join(source_errors))

    for label, mutated in mutations.items():
        if mutated == text:
            raise SystemExit(f'ERREUR auto-test workflow social utilisateur: mutation sans effet: {label}')
        if not validate_text(mutated):
            raise SystemExit(f'ERREUR auto-test workflow social utilisateur: mutation non détectée: {label}')

    print(f'OK auto-test workflow social utilisateur: {len(mutations)}/{len(mutations)} affaiblissements critiques détectés.')


def main() -> int:
    parser = argparse.ArgumentParser(description='Valide le durcissement du workflow RPC sociales utilisateur.')
    parser.add_argument('--self-test', action='store_true')
    args = parser.parse_args()

    if not WORKFLOW.is_file():
        print(f'ÉCHEC workflow social utilisateur: fichier absent: {WORKFLOW.relative_to(ROOT)}')
        return 1
    ...
    if errors:
        print(f'ÉCHEC workflow social utilisateur: {len(errors)} problème(s).')
        for error in errors:
            print('- ' + error)
        return 1

    print(
        'OK workflow social utilisateur: actions/runtimes immuables, credentials non persistés, '
        'preuves sociales et mode gratuit avant ledger/manifeste production bloquants.'
    )
    return 0
