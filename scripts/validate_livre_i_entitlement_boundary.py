...
def validate(root:Path,contract_path:Path,sql_test_path:Path,frontend_path:Path,private_edge_path:Path,reader_edge_path:Path,helper_path:Path)->list[str]:
    ...
    for key,value in {'entitlement_table':TABLE,'entitlement_client_mutation_allowed':False,'entitlement_anonymous_read_allowed':False,'owner_role_creates_entitlement':False,'client_role_assertion_allowed':False}.items():
    ...
    for marker in (".from('user_entitlements')",".eq('user_id',userId)",".eq('product_id',product.id)","service.rpc('is_sinjira_owner',{p_user_id:userId})"):
...
def self_test()->None:
    with TemporaryDirectory() as raw:
        ...
        contract.write_text(json.dumps({'full_edition':{'entitlement_table':TABLE,'entitlement_client_mutation_allowed':False,'entitlement_anonymous_read_allowed':False,'owner_role_creates_entitlement':False,'client_role_assertion_allowed':False}}),encoding='utf-8')
        ...
        helper.write_text("service.from('user_entitlements').eq('user_id',userId).eq('product_id',product.id); service.rpc('is_sinjira_owner',{p_user_id:userId});",encoding='utf-8')
...
def main()->int:
    ...
    print('OK entitlement Livre I: lecture client seule, aucune auto-attribution, accès serveur achat ou auteur sans fabriquer de droit produit.')
    return 0
