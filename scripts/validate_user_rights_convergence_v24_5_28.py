...
def main():
    ...
    if len(rows)<162: errors.append(f'Ledger: {len(rows)} migrations; V24.5.28 exige au moins ses 162 migrations historiques.')
    ...
    if bridge_row in rows and conv_row in rows and rows.index(bridge_row)>=rows.index(conv_row):
        errors.append('La migration de pont doit rester antérieure à la convergence V24.5.28.')
    ...
    if errors:
        ...
        for e in errors: print('- '+e)
        return 1
    print('OK V24.5.28: contrat historique conservé; frontière canonique, ACL et auth.uid() restent vérifiés malgré les migrations ultérieures.')
    return 0
