...
def main():
    ...
    if len(rows)!=147: errors.append(f'Ledger: {len(rows)} migrations au lieu de 147.')
    if rows.count(row)!=1 or not rows or rows[-1]!=row: errors.append('V24.5.16 doit être la dernière migration unique du ledger.')
    ...
    if errors:
        ...
        for e in errors: print('- '+e)
        return 1
    print('OK V24.5.16: 4 RPC Monde parallèle isolées, wrappers SECURITY INVOKER, identité séparée et ledger 147 synchronisé.')
    return 0
