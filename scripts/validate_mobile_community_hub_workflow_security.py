...
SELF_TEST = 'python3 scripts/validate_mobile_community_hub_workflow_security.py --self-test'
SELF_CHECK = 'python3 scripts/validate_mobile_community_hub_workflow_security.py'
VALIDATORS = (
...
GUARD_PATH = "      - 'scripts/validate_mobile_community_hub_workflow_security.py'"
PRIMARY_PATH = "      - 'scripts/validate_mobile_native_community_hub_v25.py'"
PUSH_PATHS = (
    "      - 'mobile-native/**'",
    PRIMARY_PATH,
    "      - 'scripts/validate_community_safety_v24_4_79.py'",
    "      - 'scripts/validate_public_community_v24_4_80.py'",
...
def validate_text(text: str) -> None:
    ...
    require(text.count(PRIMARY_PATH) == 2, 'Le validateur Communauté doit couvrir exactement PR et push.')
    ...
    require("python-version: '3.12.14'" in text and 'check-latest: false' in text, 'Python doit rester figé à 3.12.14 sans résolution latest.')
    require("node-version: '22.23.2'" in text and text.count('check-latest: false') == 2, 'Node doit rester figé à 22.23.2 sans résolution latest.')
...
def mutation_cases(text: str) -> tuple[tuple[str, str], ...]:
    ...
    cases = [
        ...
        ('validateur Communauté PR retiré', text.replace(PRIMARY_PATH + '\n', '', 1)),
    ...
    for index, command in enumerate(VALIDATORS):
        cases.append((f'preuve communauté {index + 1} retirée', text.replace(f'        run: {command}\n', '', 1)))
    return tuple(cases)
...
def main() -> int:
    ...
    validate_text(text)
    validate_repo_state()
    print('OK CI Communauté native: runner/Python/Node/actions immuables, Git read-only, scripts npm désactivés, aucun faux cache et aucune opération Supabase distante.')
    return 0
