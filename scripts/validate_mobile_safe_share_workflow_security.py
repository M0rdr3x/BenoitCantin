...
SELF_TEST = 'python3 scripts/validate_mobile_safe_share_workflow_security.py --self-test'
SELF_CHECK = 'python3 scripts/validate_mobile_safe_share_workflow_security.py'
HISTORICAL = (
...
TRIGGER_PATHS = (
    ...
    "      - 'scripts/validate_mobile_safe_share_workflow_security.py'",
...
def mutation_cases(text: str) -> tuple[tuple[str, str], ...]:
    ...
    return (
        ...
        ('Node large', text.replace("node-version: '22.23.2'", "node-version: '24'", 1)),
        ...
        ('chemin garde retiré', text.replace("      - 'scripts/validate_mobile_safe_share_workflow_security.py'\n", '', 1)),
        ('auto-test retiré', text.replace(f'        run: {SELF_TEST}\n', '', 1)),
        ('contrat retiré', text.replace(f'        run: {SELF_CHECK}\n', '', 1)),
        ('partage retiré', text.replace(f'        run: {HISTORICAL[0]}\n', '', 1)),
        ('challenge retiré', text.replace(f'        run: {HISTORICAL[1]}\n', '', 1)),
        ('garde secrets retiré', text.replace(f'        run: {HISTORICAL[2]}\n', '', 1)),
        ('scripts npm réactivés', text.replace(HISTORICAL[3], 'npm install --no-audit --no-fund', 1)),
        ('vault retiré', text.replace(f'        run: {HISTORICAL[4]}\n', '', 1)),
        ('typecheck retiré', text.replace(f'        run: {HISTORICAL[5]}\n', '', 1)),
        ('continue-on-error', text.replace('    timeout-minutes: 10\n', '    timeout-minutes: 10\n    continue-on-error: true\n', 1)),
        ('push Git ajouté', text.replace('      - name: Vérifier TypeScript mobile\n', '      - run: git push origin HEAD:main\n\n      - name: Vérifier TypeScript mobile\n', 1)),
        ('Supabase distant ajouté', text.replace('      - name: Vérifier TypeScript mobile\n', '      - run: supabase db push --linked\n\n      - name: Vérifier TypeScript mobile\n', 1)),
    )
...
def main() -> int:
    ...
    validate_repo_state()
    print(
        'OK CI partage mobile sûr: runner/Python/Node/actions immuables, Git read-only, '
