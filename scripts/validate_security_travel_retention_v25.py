...
def validate(root: Path) -> list[str]:
    errors: list[str] = []
    required = (MIGRATION, TEST, WORKFLOW, REVIEWED, LEDGER)
    ...
    if "cron.schedule" in mlow or "pg_cron" in mlow:
        errors.append("cette PR ne doit pas prétendre configurer un ordonnanceur production")
    ...
    for path in (str(MIGRATION), str(TEST), "scripts/validate_security_travel_retention_v25.py"):
        if path not in workflow:
            errors.append(f"déclencheur CI manquant pour {path}")

    blob = _blob_sha(root / MIGRATION)
    expected_reviewed = f"{STAMP} {NAME} {blob}"
    if expected_reviewed not in reviewed.splitlines():
        errors.append("l’empreinte Git exacte de la migration manque du lot reviewed")
...
def _self_test() -> None:
    ...
    paths = (MIGRATION, TEST, WORKFLOW, REVIEWED, LEDGER)
    ...
    cases = [
        ...
        ("hash reviewed falsifié", REVIEWED, b"41b8dc3d1b1e09c018e588755edb053e63e9904a", b"0000000000000000000000000000000000000000"),
...
def main() -> None:
    ...
    print("OK: contrat Mode Voyage rétention V25 valide")
