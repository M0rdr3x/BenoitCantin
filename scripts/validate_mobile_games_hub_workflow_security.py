...
SELF_TEST = 'python3 scripts/validate_mobile_games_hub_workflow_security.py --self-test'
SELF_CHECK = 'python3 scripts/validate_mobile_games_hub_workflow_security.py'
HISTORICAL = (
...
TRIGGER_PATHS = (
    ...
    "      - 'mobile-native/NativeGamesHub.tsx'",
    "      - 'mobile-native/NativeLibraryHub.tsx'",
    "      - 'mobile-native/NativeCharacterHub.tsx'",
    "      - 'mobile-native/NATIVE_GAMES_HUB_V25.md'",
    ...
    "      - 'scripts/validate_mobile_games_hub_workflow_security.py'",
...
def mutation_cases(text: str) -> tuple[tuple[str, str], ...]:
    ...
    return (
        ...
        ('chemin Mes parties retiré', text.replace("      - 'mobile-native/NativeGamesHub.tsx'\n", '', 1)),
        ('chemin routeur retiré', text.replace("      - 'scripts/validate_mobile_native_route_dispatch_v25.py'\n", '', 1)),
        ('chemin accueil retiré', text.replace("      - 'scripts/validate_mobile_native_home_hub_v25.py'\n", '', 1)),
        ('chemin Bibliothèque retiré', text.replace("      - 'scripts/validate_mobile_native_library_hub_v25.py'\n", '', 1)),
        ('chemin Mon personnage retiré', text.replace("      - 'scripts/validate_mobile_native_character_hub_v25.py'\n", '', 1)),
        ('chemin garde retiré', text.replace("      - 'scripts/validate_mobile_games_hub_workflow_security.py'\n", '', 1)),
        ('chemin lockfile retiré', text.replace("      - 'mobile-native/package-lock.json'\n", '', 1)),
        ('chemin documentation retiré', text.replace("      - 'mobile-native/NATIVE_GAMES_HUB_V25.md'\n", '', 1)),
        ...
        *tuple((f'preuve retirée {index + 1}', text.replace(f'        run: {command}\n', '', 1)) for index, command in enumerate(HISTORICAL)),
        ...
        ('chemin vault retiré', text.replace("      - 'mobile-native/scripts/validate-vault-mobile.mjs'\n", '', 1)),
        ('chemin navigation retiré', text.replace("      - 'scripts/validate_mobile_navigation_boundary_v25.py'\n", '', 1)),
        ('chemin workflow retiré', text.replace("      - '.github/workflows/sinjira-mobile-native-games-hub-v25.yml'\n", '', 1)),
...
def main() -> int:
    ...
    validate_text(text)
    validate_repo_state()
    print('OK CI Mes parties natif: runner/Python/Node/actions immuables, Git read-only, scripts npm désactivés, aucun faux cache et aucune opération Supabase distante.')
    return 0
