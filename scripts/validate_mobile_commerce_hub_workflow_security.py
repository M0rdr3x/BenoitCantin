...
SELF_TEST = 'python3 scripts/validate_mobile_commerce_hub_workflow_security.py --self-test'
SELF_CHECK = 'python3 scripts/validate_mobile_commerce_hub_workflow_security.py'
HISTORICAL = (
...
TRIGGER_PATHS = (
    "      - 'mobile-native/NativeCommerceHub.tsx'",
    "      - 'mobile-native/NativeHomeHub.tsx'",
    "      - 'mobile-native/NativeModuleRouter.tsx'",
    "      - 'mobile-native/NATIVE_COMMERCE_HUB_V25.md'",
    "      - 'scripts/validate_mobile_native_commerce_hub_v25.py'",
    ...
    "      - 'scripts/validate_mobile_commerce_hub_workflow_security.py'",
...
def mutation_cases(text: str) -> tuple[tuple[str, str], ...]:
    ...
    return (
        ...
        ('chemin Commerce retiré', text.replace("      - 'mobile-native/NativeCommerceHub.tsx'\n", '', 1)),
        ...
        ('chemin garde retiré', text.replace("      - 'scripts/validate_mobile_commerce_hub_workflow_security.py'\n", '', 1)),
        ('chemin lockfile retiré', text.replace("      - 'mobile-native/package-lock.json'\n", '', 1)),
        ('chemin documentation retiré', text.replace("      - 'mobile-native/NATIVE_COMMERCE_HUB_V25.md'\n", '', 1)),
        ...
        *tuple((f'preuve retirée {index + 1}', text.replace(f'        run: {command}\n', '', 1)) for index, command in enumerate(HISTORICAL)),
        ...
        ('chemin workflow retiré', text.replace("      - '.github/workflows/sinjira-mobile-native-commerce-hub-v25.yml'\n", '', 1)),
...
def main() -> int:
    ...
    validate_text(text)
    validate_repo_state()
    print('OK CI Commerce natif: runner/Python/Node/actions immuables, Git read-only, scripts npm désactivés, aucun faux cache, paiement local ou opération Supabase distante.')
    return 0
