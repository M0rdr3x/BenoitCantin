...
SELF_TEST = 'python3 scripts/validate_mobile_hub_destinations_workflow_security.py --self-test'
SELF_CHECK = 'python3 scripts/validate_mobile_hub_destinations_workflow_security.py'
VALIDATORS = (
...
GUARD_PATH = "      - 'scripts/validate_mobile_hub_destinations_workflow_security.py'"
PUSH_PATHS = (
    "      - 'compte/*.html'",
    "      - 'mobile-native/**'",
...
def mutation_cases(text: str) -> tuple[tuple[str, str], ...]:
    ...
    for index, command in enumerate(VALIDATORS):
        cases.append((f'preuve destinations {index + 1} retirée', text.replace(f'        run: {command}\n', '', 1)))
    return tuple(cases)
...
def main() -> int:
    ...
    validate_text(text)
    validate_repo_state()
    print('OK CI destinations hubs natifs: runner/Python/Node/actions immuables, Git read-only, scripts npm désactivés, aucun faux cache et aucune opération Supabase distante.')
    return 0
