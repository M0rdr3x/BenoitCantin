#!/usr/bin/env python3
"""Security contract for the UUID-free game contribution CI workflow."""
...
WORKFLOW = Path('.github/workflows/sinjira-game-contribution-v24-5-53.yml')
...
def validate_text(text: str) -> None:
    required = [
        ...
        'python scripts/validate_game_contribution_workflow_security.py --self-test',
        'python scripts/validate_game_contribution_workflow_security.py',
        'python scripts/validate_game_contribution_v24_5_53.py',
...
def self_test(text: str) -> None:
    mutations = {
        ...
        'opération Supabase distante': text.replace(
            '      - name: Vérifier V24.5.53\n',
            '      - name: Mauvaise opération distante\n        run: supabase db push --linked\n\n      - name: Vérifier V24.5.53\n',
            1,
        ),
        'auto-test retiré': text.replace(
            '      - name: Auto-tester le contrat CI contribution jeu\n'
            '        run: python scripts/validate_game_contribution_workflow_security.py --self-test\n\n',
            '',
            1,
        ),
        'contrôle UUID retiré': text.replace('          python scripts/validate_game_contribution_v24_5_53.py\n', '', 1),
...
def main() -> None:
    ...
    if args.self_test:
        ...
        validate_text(text)
        print('OK: contrat CI contribution jeu sans UUID respecté')
