...
def main():
    ...
    require(migration,[
        'create table if not exists public.dating_profiles','create table if not exists public.dating_preferences',
        'create table if not exists public.dating_connections','create table if not exists public.dating_messages',
        'alter table public.dating_profiles enable row level security','alter table public.dating_messages enable row level security',
        ...
        "private.dating_age(p_user_id) >= 18","s.relationship_status='single'",'p.serious_intent_confirmed is true',
        "p.single_confirmed_at >= now()-interval '90 days'",'public.dating_compatibility_candidates','public.dating_request_conversation',
        'public.dating_respond_connection','public.dating_send_message','public.dating_set_photo_consent','v_my<10 or v_their<10',
        "notification_type,title,body","'dating'","'/compte/rencontres.html'",
        "array['core_value','conflict_style','decision_style','sociability','trust_style','natural_role']"
    ],'migration Rencontres')
    forbid(migration,['photo_path text','image_path text','attachment','pg_net','http_post','send_email','twilio','stripe','openai','anthropic'],'migration Rencontres')

    require(safety,[
        'public.dating_close_connection','public.dating_block_connection',
        "status='closed'",'a_photo_consent=false','b_photo_consent=false',
    ...
    ],'sécurité contact Rencontres')
    forbid(safety,['select * from public.social_profiles','return v_other_user','pg_net','http_post','send_email'],'sécurité contact Rencontres')
    ...
    require(js,[
        'requireCommunityUser',"s.from('dating_profiles')","s.from('dating_preferences')", "s.rpc('dating_self_status')",
        "s.rpc('dating_compatibility_candidates'", "s.rpc('dating_request_conversation'", "s.rpc('dating_respond_connection'",
        "s.rpc('dating_send_message'", "s.rpc('dating_set_photo_consent'", "s.rpc('dating_close_connection'", "s.rpc('dating_block_connection'",
        'data-dating-close','data-dating-block','photo_unlock_available','identity_revealed','avatarUrl(item.revealed_avatar_path)',
        'Vous : ${Number(item.my_message_count||0)}/10','use_registry_answers'
    ],'runtime Rencontres')
    forbid(js,[
        ".from('dating_connections')", ".from('dating_messages')", '.storage.upload(', 'functions.invoke(', 'fetch("http', "fetch('http",
        'openai','anthropic','stripe','data-like','swipe'
    ],'runtime Rencontres')
    ...
    require(ledger,[
        '20260819221038 sinjira_v24_4_74_compatibility_dating_private',
