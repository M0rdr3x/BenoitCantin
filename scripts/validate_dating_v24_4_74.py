...
def main():
    ...
    chat=CHAT.read_text('utf-8',errors='ignore')
    harden=HARDEN.read_text('utf-8',errors='ignore')
    ...
    require(migration,[
        'create table if not exists public.dating_profiles','create table if not exists public.dating_introductions',
        'create table if not exists public.dating_photo_reveal_consents',"public.sinjira_age_band(p_user_id)='adult'",
        "array['Célibataire','Divorcé(e)','Veuf / veuve']",'public.social_is_blocked(p_a,p_b)',
        'public.has_accepted_community_rules(p_user_id)','not public.social_is_suspended(p_user_id)',
        'private.dating_safe_questionnaire_traits','source_purged_at is null',"'scale_reserved_social'", "'scale_flexible_rigid'",
        'sent_n>=10 and received_n>=10 and mine and theirs',"'threshold',10",
    ...
    require(chat,[
        'create table if not exists public.dating_messages','revoke all on table public.dating_messages from public,anon,authenticated',
        'create or replace function public.dating_conversation','create or replace function public.dating_send_message',
        'create or replace function public.dating_report_message','from public.dating_messages m',
        "'other_user_id',null","'other_pseudo','Membre compatible'",
        "'mine',m.sender_user_id=uid","'body',m.body","'created_at',m.created_at",
        "'other_avatar_path',case when unlocked then avatar else null end",
        "'other_pseudo',case when unlocked then pseudo else null end"
    ],'chat aveugle Rencontres')
    conversation=function_body(chat,'public.dating_conversation')
    forbid(conversation,["'sender_user_id'","'recipient_user_id'","'pseudo'","'avatar_path'"],'sortie conversation aveugle')
    ...
    require(page,['<meta content="noindex,nofollow" name="robots"','Rencontres par compatibilité','18+ uniquement','Pas de swipe','Aucune photo dans la découverte','10 messages de chaque côté','data-dating-form','data-dating-recommendations','data-dating-introductions','data-dating-delete','sinjira-dating-v24-4-74.js?v=24.4.74'],'page Rencontres')
    forbid(page,['OnlyFans','Tinder','Bumble'],'page Rencontres')

    require(js,[
        "rpc('dating_my_eligibility')","from('dating_profiles').select('*').eq('user_id',user.id)","rpc('dating_save_profile'",
        "rpc('dating_recommendations'",'row.recommendation_token',"rpc('dating_request_introduction',{p_recommendation_token:recommendationToken})",
        'Compatibilité proposée','Ni pseudo, ni identifiant communautaire, ni photo ne sont révélés',
        "rpc('dating_respond_introduction'","rpc('dating_conversation'","rpc('dating_send_message'","rpc('dating_report_message'",
        "rpc('dating_photo_reveal_status'","rpc('dating_request_photo_reveal'","rpc('dating_revoke_photo_reveal'","rpc('dating_delete_my_profile')",
        'Conversation anonyme ouverte','Écrire sans envoyer de photo ni de fichier','avatarUrl(data.other_avatar_path)'
    ],'runtime Rencontres')
