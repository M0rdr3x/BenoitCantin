...
def validate_text(text: str) -> None:
    ...
    require('runs-on: ubuntu-24.04' in text, 'Le recovery drill doit rester figé sur Ubuntu 24.04.')
    require('ubuntu-latest' not in active, 'Le runner mobile ubuntu-latest est interdit.')
    ...
    require(positions == sorted(positions), 'Ordre recovery attendu: Python, garde, CLI, start, reset, pgTAP, Auth local, stop.')
...
def main() -> int:
    ...
    print(
        'OK recovery drill: Ubuntu/Python/actions/CLI immuables, credentials Git non persistés, '
